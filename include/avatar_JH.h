...
public:
    ...
    //------------------------------------------------------------------JH
    void computeLBIKcontrol_HQP();
    //
    const int hierarchy_num_lbikhqp_ = 2;
    const int variable_size_lbikhqp_ = 12;
    const int constraint_size1_lbikhqp_ = 12;
    const int constraint_size2_lbikhqp_[2] = {0,6};
    const int control_size_lbikhqp_[2] = {6, 6}; //1: CAM control, 2: init pose // original number -> 6 (DG)

    double w1_lbikhqp_[2];
    double w2_lbikhqp_[2];
    double w3_lbikhqp_[2];

    Eigen::MatrixXd H_lbikhqp_[2], A_lbikhqp_[2];
    Eigen::MatrixXd J_lbikhqp_[2];
    Eigen::VectorXd g_lbikhqp_[2], u_dot_lbikhqp_[2], qpres_lbikhqp_, ub_lbikhqp_[2],lb_lbikhqp_[2], ubA_lbikhqp_[2], lbA_lbikhqp_[2];
    Eigen::VectorXd q_dot_lbikhqp_[2];

    int control_joint_idx_lbikhqp_[12];
    int last_solved_hierarchy_num_lbikhqp_;
    //

//    Eigen::VectorQd motion_q_lbikhqp_; // JH
//    Eigen::VectorQd motion_q_dot_lbikhqp_; // JH
//    Eigen::VectorQd motion_q_lbikhqp_pre_; // JH
    Eigen::Vector3d lfoot_control_point_lbikhqp_, rfoot_control_point_lbikhqp_; // JH

    bool first_loop_lbikhqp_;//JH
    bool first_loop_lbikhqp_2; //JH

    Eigen::Vector12d q_des_lb_;//JH

    Eigen::VectorQd q_des_slow_;//JH
    Eigen::VectorQd q_des_fast_;//JH
    Eigen::Isometry3d pelv_trajectory_float_fast_;//JH
    Eigen::Isometry3d pelv_trajectory_float_slow_;//JH

    std::vector<CQuadraticProgram> QP_lbik_hqp_;//JH

    Eigen::Isometry3d master_lfoot_pose_;//JH
    Eigen::Isometry3d master_rfoot_pose_;//JH
    Eigen::Isometry3d master_lfoot_pose_pre_;//JH
    Eigen::Isometry3d master_rfoot_pose_pre_;//JH
    ...
    Eigen::MatrixXd J_hqpik_[4], J_temp_, J_temp_2_;

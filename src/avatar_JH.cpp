...
void AvatarController::setGains()
{
    ...
    joint_limit_l_(2) = -80 * DEG2RAD;//left hip pitch
    joint_limit_h_(2) = 80 * DEG2RAD;
    joint_limit_l_(3) = -180 * DEG2RAD;//left knee pitch
    joint_limit_h_(3) = 180 * DEG2RAD;
    joint_limit_l_(4) = -80 * DEG2RAD;//left ankle pitch
    joint_limit_h_(4) = 80 * DEG2RAD;
    joint_limit_l_(5) = -80 * DEG2RAD;//left ankle roll
    ...
    joint_limit_l_(8) = -80 * DEG2RAD;//right hip pitch
    joint_limit_h_(8) = 80 * DEG2RAD;
    joint_limit_l_(9) = -180 * DEG2RAD;//right knee pitch
    joint_limit_h_(9) = 180 * DEG2RAD;
    joint_limit_l_(10) = -80 * DEG2RAD;//right ankle pitch
    joint_limit_h_(10) = 80 * DEG2RAD;
    joint_limit_l_(11) = -80 * DEG2RAD;//right ankle roll
    ...
    else if (rd_.tc_.mode == 11)
    {
        ...
        if (walking_enable_ == true)
        {
            ...
            if (current_step_num_ < total_step_num_)
            {
                ...
                //fig4
                JH_q_ik << desired_q_fast_(0)* RAD2DEG << ","
                     << desired_q_fast_(1)* RAD2DEG << ","
                     << desired_q_fast_(2)* RAD2DEG << ","
                     << desired_q_fast_(3)* RAD2DEG << ","
                     << desired_q_fast_(4)* RAD2DEG << ","
                     << desired_q_fast_(5)* RAD2DEG << ","
                     << desired_q_fast_(6)* RAD2DEG << ","
                     << desired_q_fast_(7)* RAD2DEG << ","
                     << desired_q_fast_(8)* RAD2DEG << ","
                     << desired_q_fast_(9)* RAD2DEG << ","
                     << desired_q_fast_(10)* RAD2DEG << ","
                     << desired_q_fast_(11)* RAD2DEG << endl;
                // update
    ...
    else if (rd_.tc_.mode == 11)
    {
        ...
        if (walking_enable_ == true)
        {
            if (current_step_num_ < total_step_num_)
            {
                ...
                computeLBIKcontrol_HQP(lfoot_trajectory_float_slow_, rfoot_trajectory_float_slow_, motion_q_, motion_q_dot_);
...
///===========================================================================================================================================================================
/// LBIKHQP
void AvatarController::computeLBIKcontrol_HQP(Eigen::Isometry3d lfoot_t_des, Eigen::Isometry3d rfoot_t_des,  Eigen::VectorQd &desired_q, Eigen::VectorQd &desired_q_dot)
{
// const int hierarchy_num_lbikhqp_ = 2;
// const int variable_size_lbikhqp_ = 12;
// const int constraint_size1_lbikhqp_ = 12;
// const int constraint_size2_lbikhqp_[2] = {12,18}; // inequality constraint size --> 상위 control size 합만큼 늘어남
...
///==================================================================================================================================
    ...
    JH_lf << lfoot_t_des.translation()(0) << " , " << lfoot_t_des.translation()(1) << " , " << lfoot_t_des.translation()(2) << endl;
    JH_rf << rfoot_t_des.translation()(0) << " , " << rfoot_t_des.translation()(1) << " , " << rfoot_t_des.translation()(2) << endl;
...
/// ======
    ...
    J_lbikhqp_[0].block(0,0,3,6) = J_temp_.block(3,6,3,6); //lfoot_position

    J_temp_.setZero(6, MODEL_DOF_VIRTUAL);
    RigidBodyDynamics::CalcPointJacobian6D(model_d_, pre_desired_q_qvqd_, rd_.link_[Right_Foot].id, zero3, J_temp_, false);
    J_lbikhqp_[0].block(3,6,3,6) = J_temp_.block(3,12,3,6); //rfoot_position
    ...
    u_dot_lbikhqp_[0] =  500*error_v_leg_pos; //leg position error
...
/// ======
    J_temp_.setZero(6, MODEL_DOF_VIRTUAL);
    RigidBodyDynamics::CalcPointJacobian6D(model_d_, pre_desired_q_qvqd_, rd_.link_[Left_Foot].id, zero3, J_temp_, false);
    J_lbikhqp_[1].block(0,0,3,6) = J_temp_.block(0,6,3,6); //lfoot_orientation
    ...
    u_dot_lbikhqp_[1] = 500*error_w_leg_pos; // leg orientation error
...
/// solve
    ...
    for (int i = 0; i < hierarchy_num_lbikhqp_; i++)
    {
        ...
        g3 = -motion_q_dot_pre_.segment(0, variable_size_lbikhqp_) * (1 / dt_) * (1 / dt_);
        ...
        for (int j = 0; j < constraint_size1_lbikhqp_; j++)
        {
        ...
        if (i < 2)
        {
            A_lbikhqp_[i].block(higher_task_equality_num_lbikhqp_, 0, 6, variable_size_lbikhqp_) = J_lbikhqp_[0]; //position
            A_lbikhqp_[i].block(higher_task_equality_num_lbikhqp_ + 6, 0, 6, variable_size_lbikhqp_) = J_lbikhqp_[1];  //orientation
            for (int j = 0; j < 3; j++)
            {
                //linear velocity limit
                lbA_lbikhqp_[i](higher_task_equality_num_lbikhqp_ + j) = -2;
                ubA_lbikhqp_[i](higher_task_equality_num_lbikhqp_ + j) = 2;
                lbA_lbikhqp_[i](higher_task_equality_num_lbikhqp_ + j + 3) = -2;
                ubA_lbikhqp_[i](higher_task_equality_num_lbikhqp_ + j + 3) = 2;
                //angular velocity limit
                lbA_lbikhqp_[i](higher_task_equality_num_lbikhqp_ + j + 6) = -2*M_PI;
                ubA_lbikhqp_[i](higher_task_equality_num_lbikhqp_ + j + 6) = 2*M_PI;
                lbA_lbikhqp_[i](higher_task_equality_num_lbikhqp_ + j + 9) = -2*M_PI;
                ubA_lbikhqp_[i](higher_task_equality_num_lbikhqp_ + j + 9) = 2*M_PI;
            }
        }
    ...
    for (int i = 0; i < variable_size_lbikhqp_; i++)
    {
        desired_q_dot(i) = q_dot_lbikhqp_[last_solved_hierarchy_num_lbikhqp_](i);
...
//        control_joint_idx_lbikhqp_[11] = 11; //right ankle roll
            ...
            // 확실하지않음 --> hqpik 에 있던 것 따라함 j: 0 --> 다리같은경우 힙yaw +-20도 힙롤--> ㄱ두이없어도ㄱㅊ +- 90도정도 힙피치-->+- 90도 kneepitch 왼오른쪽다름-->왼쪽0도쫙폈을떄 구브리는것-양수 0~150 deg // right --> -150~0  발목-->+-90
...
//    }
    // fig2
    JH_q_2 << desired_q(control_joint_idx_lbikhqp_[0]) << ","
<< desired_q(control_joint_idx_lbikhqp_[1]) << ","
<< desired_q(control_joint_idx_lbikhqp_[2]) << ","
<< desired_q(control_joint_idx_lbikhqp_[3]) << ","
<< desired_q(control_joint_idx_lbikhqp_[4]) << ","
<< desired_q(control_joint_idx_lbikhqp_[5]) << ","
<< desired_q(control_joint_idx_lbikhqp_[6]) << ","
<< desired_q(control_joint_idx_lbikhqp_[7]) << ","
<< desired_q(control_joint_idx_lbikhqp_[8]) << ","
<< desired_q(control_joint_idx_lbikhqp_[9]) << ","
<< desired_q(control_joint_idx_lbikhqp_[10]) << ","
<< desired_q(control_joint_idx_lbikhqp_[11]) << endl;
    // fig3
    JH_q_3 << desired_q_dot(control_joint_idx_lbikhqp_[0])  << ","
<< desired_q_dot(control_joint_idx_lbikhqp_[1])  << "," << desired_q_dot(control_joint_idx_lbikhqp_[2])  << ","           << desired_q_dot(control_joint_idx_lbikhqp_[3])  << ","           << desired_q_dot(control_joint_idx_lbikhqp_[4])  << ","           << desired_q_dot(control_joint_idx_lbikhqp_[5])  << ","           << desired_q_dot(control_joint_idx_lbikhqp_[6])  << ","           << desired_q_dot(control_joint_idx_lbikhqp_[7])  << ","           << desired_q_dot(control_joint_idx_lbikhqp_[8])  << ","           << desired_q_dot(control_joint_idx_lbikhqp_[9])  << ","           << desired_q_dot(control_joint_idx_lbikhqp_[10])  << ","           << desired_q_dot(control_joint_idx_lbikhqp_[11])  << endl;

    //fig5
  JH_q_lbik  << desired_q(0) *RAD2DEG<< " , "
             << desired_q(1) *RAD2DEG<< " , "
             << desired_q(2) *RAD2DEG<< " , "
             << desired_q(3) *RAD2DEG<< " , "
             << desired_q(4) *RAD2DEG<< " , "
             ...
             << desired_q(8) *RAD2DEG<< " , "
             << desired_q(9) *RAD2DEG<< " , "
             << desired_q(10)*RAD2DEG<< " , "
             << desired_q(11)*RAD2DEG<< endl;

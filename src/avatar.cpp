...
AvatarController::AvatarController(RobotData &rd) : rd_(rd)
{
    ...
    first_loop_leg_hqpik_ = true;
    first_loop_leg_qpik_ = true;
}
...
void AvatarController::setGains()
{
    ...
    torque_nm2cnt_(0) = 3.0; // leftLeg
    torque_nm2cnt_(1) = 4.3;
    torque_nm2cnt_(2) = 3.8;
    torque_nm2cnt_(3) = 3.46;
    torque_nm2cnt_(4) = 4.5;
    torque_nm2cnt_(5) = 6.0;
    torque_nm2cnt_(6) = 3.0; // rightLeg
    torque_nm2cnt_(7) = 4.3;
    torque_nm2cnt_(8) = 3.8;
    torque_nm2cnt_(9) = 3.46;
    torque_nm2cnt_(10) = 4.5;
    torque_nm2cnt_(11) = 6.0;
    torque_nm2cnt_(12) = 3.3; // Waist
    torque_nm2cnt_(13) = 3.3;
    torque_nm2cnt_(14) = 3.3;  // upperbody
    torque_nm2cnt_(15) = 15.5; // shoulder2
    torque_nm2cnt_(16) = 15.5; // shoulder1
    torque_nm2cnt_(17) = 15.5; // shoulder2
    torque_nm2cnt_(18) = 15.5; // arm
    torque_nm2cnt_(19) = 42.0; // Elbow
    torque_nm2cnt_(20) = 42.0; // Forearm
    torque_nm2cnt_(21) = 95.0; // wrist
    torque_nm2cnt_(22) = 95.0;
    torque_nm2cnt_(23) = 95.0; // head
    torque_nm2cnt_(24) = 95.0;
    torque_nm2cnt_(25) = 15.5; // shoulder2
    torque_nm2cnt_(26) = 15.5; // shoulder1
    torque_nm2cnt_(27) = 15.5; // shoulder2
    torque_nm2cnt_(28) = 15.5; // arm
    torque_nm2cnt_(29) = 42.0; // Elbow
...
void AvatarController::computeSlow()
{
    ...
    if (rd_.tc_.mode == 10)
    {
        ...
        // {
        //    cout << "l_ft: "<<rd_.LF_FT.transpose() << "\n r_ft_: " << rd_.RF_FT.transpose() << endl;
        //    cout <<"l_ft_force_norm: "<<rd_.LF_FT.segment(0, 3).norm()<<endl;
        //    cout <<"r_ft_force_norm: "<<rd_.RF_FT.segment(0, 3).norm()<<endl;
        // }
    ...
    else if (rd_.tc_.mode == 11)
    {
        ...
        if (walking_enable_ == true) // 31~180us
        {
            ...
            if (current_step_num_ < total_step_num_)
            {
                ...
                if (walking_tick_mj < 1.0 * hz_)
                {
                    ...
                    for (int i = 0; i < 12; i++)
                    {
                        ref_q_(i) = DyrosMath::cubic(walking_tick_mj, 0, 1.0 * hz_, Initial_ref_q_(i), ref_q_(i), 0.0, 0.0);
                    }
                    // for waist
                ...
                Vector12d p_gaussian;
                double first_smoothing_gain;
                ...
                for (int i = 0; i < 12; i++)
                {
                    ...
                    torque_lower_(i) = DyrosMath::minmax_cut( torque_lower_(i), -500.0, 500.0);
        ...
        for (int i = 12; i < MODEL_DOF; i++)
        {
            ...
            // torque_upper_(i) = (kp_joint_(i) * (Initial_ref_upper_q_(i) - rd_.q_(i)) + kv_joint_(i) * (0.0 - rd_.q_dot_(i)) + 1.0 * Gravity_MJ_fast_(i));
            torque_upper_(i) = DyrosMath::minmax_cut( torque_upper_(i), -500.0, 500.0);
        ...
        // for(int i = 0; i<MODEL_DOF; i++)
        // {
        //     torque_command(i) = DyrosMath::minmax_cut( torque_lower_(i)+ torque_upper_(i), -500.0, 500.0);
        // }
    ...
    else if (rd_.tc_.mode == 13)
    {
        ...
        else
        {
            ...
            pre_time_computeslow_ = current_time_computeslow_; // dg test
        }
...
void AvatarController::computeFast()
{
    if (rd_.tc_.mode == 10)
    {
        if (initial_flag == 1)
        {
            ...
            // loadLstmWeights(left_leg_mob_lstm_, "/home/dg/catkin_ws/src/tocabi_avatar/lstm_sim/weights/left_leg/friction_model_error_noise/");
            // loadLstmMeanStd(left_leg_mob_lstm_, "/home/dg/catkin_ws/src/tocabi_avatar/lstm_sim/mean_std/left_leg/friction_model_error_noise/");

            initializeLegLSTM(right_leg_mob_lstm_);
            // loadLstmWeights(right_leg_mob_lstm_, "/home/dg/catkin_ws/src/tocabi_avatar/lstm_sim/weights/right_leg/friction_model_error_noise/");
            // loadLstmMeanStd(right_leg_mob_lstm_, "/home/dg/catkin_ws/src/tocabi_avatar/lstm_sim/mean_std/right_leg/friction_model_error_noise/");
            initial_flag = 2;
        }
    }
    else if (rd_.tc_.mode == 11)
    ...
    else if (rd_.tc_.mode == 12)
    {
        if (initial_flag == 1)
        {
            ...
            // loadLstmMeanStd(right_leg_mob_lstm_, "/home/dyros/catkin_ws/src/tocabi_avatar/lstm_tocabi/mean_std/right_leg/");
            if (atb_grav_update_ == false)
            {
                atb_grav_update_ = true;
                Gravity_MJ_ = Gravity_MJ_local;
...
void AvatarController::initWalkingParameter()
{
    ...
    torque_sim_jts_.setZero();
    torque_current_elmo_.setZero();
    torque_from_l_ft_.setZero();
    torque_from_r_ft_.setZero();
    torque_from_l_ft_lpf_.setZero();
    torque_from_r_ft_lpf_.setZero();

    l_ft_wo_fw_.setZero();
    r_ft_wo_fw_.setZero();
    l_ft_wo_fw_lpf_.setZero();
    r_ft_wo_fw_lpf_.setZero();
    ...
    mob_residual_external_.setZero(6);
    mob_integral_external_.setZero(6);
    ...
    hmd_lupperarm_vel_.setZero();
    hmd_lhand_vel_.setZero();
    hmd_rshoulder_vel_.setZero();
    hmd_rupperarm_vel_.setZero();
    hmd_rhand_vel_.setZero();
    hmd_chest_vel_.setZero();
    hmd_pelv_vel_.setZero();

    hmd_chest_vel_thread_.setZero();
    hmd_chest_vel_slow_.setZero();
    hmd_chest_vel_slow_lpf_.setZero();
...
void AvatarController::getRobotData()
{
    ...
    // RigidBodyDynamics::CompositeRigidBodyAlgorithm(model_global_, rd_.q_virtual_, A_temp, true);
    // A_mat_ = A_temp;
    ...
    Matrix6d adjoint_lfoot_2_global, adjoint_rfoot_2_global, adjoint_pelv_2_global;
    adjoint_lfoot_2_global.setZero();
    adjoint_lfoot_2_global.block(0, 0, 3, 3) = pelv_transform_current_from_global_.linear().transpose() * lfoot_transform_current_from_global_.linear();
    adjoint_lfoot_2_global.block(3, 3, 3, 3) = pelv_transform_current_from_global_.linear().transpose() * lfoot_transform_current_from_global_.linear();
    adjoint_rfoot_2_global.setZero();
    adjoint_rfoot_2_global.block(0, 0, 3, 3) = pelv_transform_current_from_global_.linear().transpose() * rfoot_transform_current_from_global_.linear();
    adjoint_rfoot_2_global.block(3, 3, 3, 3) = pelv_transform_current_from_global_.linear().transpose() * rfoot_transform_current_from_global_.linear();
    adjoint_pelv_2_global.setZero();
...
void AvatarController::floatingBaseMOB()
{
    ...
    RigidBodyDynamics::CompositeRigidBodyAlgorithm(model_global_, q_virtual_Xd_global_noise_, A_temp, false);
    ...
    adjt_global2pelv.block(0, 0, 3, 3) = rd_.link_[Pelvis].rotm.transpose();
    adjt_global2pelv.block(3, 3, 3, 3) = rd_.link_[Pelvis].rotm.transpose();

    nonlinear_torque_ = nonlinear_torque_temp;
    // nonlinear_torque_.segment(0, 6) = adjt_global2pelv * nonlinear_torque_.segment(0, 6);    //local frame
    ...
    // rfoot_ft_sensor_offset_  << 0.03, 0, -0.145;
    // rfoot_ft_sensor_offset_(2) += -0.0135;
    // RigidBodyDynamics::UpdateKinematicsCustom(model_global_, &rd_.q_virtual_, &q_dot_virtual_c, &q_ddot_virtual_c); // global frame is fixed to the pelvis frame
    ...
    {
        // cout<<"base_velocity: "<<base_velocity<<endl;
        //     cout<<"momentumObserverFbInternal time: "<< std::chrono::duration_cast<std::chrono::nanoseconds>(t8 - t7).count() <<endl;
        // cout<<"sqrt(-1.0):"<< isnan(sqrt(-1.0))<<endl;
        // cout<<"sqrt(-1.0) == -nanf64:"<<(sqrt(-1.0) == -nanf64)<<endl;
        // // cout<<"test: "<<isnan(sqrt(-1.0))<<endl;
        // cout<<"mob_residual_internal_: "<<mob_residual_internal_.segment(0, 6).transpose() <<endl;
        // cout<<"current_torque: "<<current_torque.segment(0, 6).transpose() <<endl;
        // cout<<"nonlinear_torque_: "<<nonlinear_torque_.transpose() <<endl;
        // cout<<"gravity_torque_temp.segment(3, 3): "<<gravity_torque_temp.segment(3, 3)<<endl;
        ...
        // cout<<"rd_.imu_lin_acc: "<<rd_.imu_lin_acc.transpose()<<endl;
        // cout<<"q_ddot_virtual_: "<<rd_.q_ddot_virtual_.segment(0, 6).transpose()<<endl;
    }
    ...
    {
        left_leg_mob_lstm_.atb_lstm_output_update_ = true;
        estimated_model_unct_torque_slow_ = estimated_model_unct_torque_thread_;
        estimated_model_unct_torque_variance_slow_ = estimated_model_unct_torque_variance_thread_;
        left_leg_mob_lstm_.atb_lstm_output_update_ = false;
    }

    if (walking_tick_mj == 0)
    {
        estimated_model_unct_torque_slow_lpf_ = estimated_model_unct_torque_slow_;
    }
    ...
    // RigidBodyDynamics::CalcPointJacobian6D(model_local_, q_virtual_c, rd_.link_[Pelvis].id, Vector3d::Zero(), j_pelv_temp, false);
    ...
    // Vector6d wrench_appied_at_sh, wrench_appied_at_pelv;
    // wrench_appied_at_sh << 10.0, 20.0, -50.0, 0.0, 0.0, 0.0;
    ...
    if (walking_tick_mj % 200 == 0)
    {
        ...
        // cout<<"torque_from_r_ft_: \n"<< (torque_from_r_ft_).segment(0, 12).transpose() <<endl;
        // cout<<"mob_residual_error - torque_from_ft: \n" << (mob_residual_internal_- torque_from_l_ft_ - torque_from_r_ft_).segment(0, 12).transpose() <<endl;
        // cout<<"left_foot_ext_torque: "<< (jac_lfoot_.block(0, 6, 6, MODEL_DOF).transpose()*adjoint_lfoot_2_global*l_ft_).transpose() <<endl;
        // cout<<"right_foot_ext_torque: "<< (jac_rfoot_.block(0, 6, 6, MODEL_DOF).transpose()*adjoint_rfoot_2_global*r_ft_).transpose() <<endl;
        // cout<<"internal residual error: "<< (jac_lfoot_.block(0, 6, 6, MODEL_DOF).transpose()*adjoint_lfoot_2_global*l_ft_).transpose() + (jac_rfoot_.block(0, 6, 6, MODEL_DOF).transpose()*adjoint_rfoot_2_global*r_ft_).transpose() - mob_residual_internal_.transpose()<<endl;

        // cout<<"l_ft_: "<< (adjoint_lfoot_2_global*l_ft_).transpose() <<endl;
        // cout<<"r_ft_: "<< (adjoint_rfoot_2_global*r_ft_).transpose() <<endl;
        // cout<<"l_ft_residual: "<< ( (jac_lfoot_.block(0, 6, 6, MODEL_DOF)*jac_lfoot_.block(0, 6, 6, MODEL_DOF).transpose()).inverse()*jac_lfoot_.block(0, 6, 6, MODEL_DOF)*mob_residual_internal_ ).transpose() <<endl;
        // cout<<"r_ft_residual: "<< ( (jac_rfoot_.block(0, 6, 6, MODEL_DOF)*jac_rfoot_.block(0, 6, 6, MODEL_DOF).transpose()).inverse()*jac_rfoot_.block(0, 6, 6, MODEL_DOF)*mob_residual_internal_ ).transpose() <<endl;
        // cout<<"lfoot interal_wrench_error: "<<(adjoint_lfoot_2_global*l_ft_).transpose() - ( (jac_lfoot_.block(0, 6, 6, MODEL_DOF)*jac_lfoot_.block(0, 6, 6, MODEL_DOF).transpose()).inverse()*jac_lfoot_.block(0, 6, 6, MODEL_DOF)*mob_residual_internal_ ).transpose() <<endl;
        // cout<<"rfoot interal_wrench_error: "<<(adjoint_rfoot_2_global*r_ft_).transpose() - ( (jac_rfoot_.block(0, 6, 6, MODEL_DOF)*jac_rfoot_.block(0, 6, 6, MODEL_DOF).transpose()).inverse()*jac_rfoot_.block(0, 6, 6, MODEL_DOF)*mob_residual_internal_ ).transpose() <<endl;
        // cout<<"nonlinear_torque_ : "<<nonlinear_torque_.transpose() << endl;
...
void AvatarController::collisionIsolation()
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        // left leg
        if (abs(estimated_ext_torque_lstm_(i)) > threshold_joint_torque_collision_(i))
        {
    ...
    if( left_leg_collision_detected_link_ != 0 && check_left_swing_foot_ && estimated_ext_force_lfoot_lstm_(2) < foot_normal_force_threshold)
    {
        ...
        else if (left_leg_collision_detected_link_ == 4)
        {
    ...
    if( right_leg_collision_detected_link_ != 0 && check_right_swing_foot_ && estimated_ext_force_rfoot_lstm_(2) < foot_normal_force_threshold)
    {
        ...
        else if (right_leg_collision_detected_link_ == 4)
        {
        ...
        if(right_leg_collision_cnt_[0]>= continuous_filter_window || right_leg_collision_cnt_[1] >= continuous_filter_window || right_leg_collision_cnt_[2] >= continuous_filter_window )
        {
            ...
            if (collision_detection_flag_ == false)
            {
                ...
                cout << "estimated_ext_torque_lstm_: \n"
                     << estimated_ext_torque_lstm_.segment(6, 6).transpose() << endl;

                calculateFootStepTotal_reactive(rfoot_support_current_.translation()(0), true);
                collision_detection_flag_ = true;
            }
...
void AvatarController::collisionIdentification()
{
    if (left_leg_collision_detected_link_ == 3)
    {
        ...
        MatrixXd J_temp, left_leg_thigh_jac_3_3;
        ...
        I3.setIdentity(3, 3);
        I6.setIdentity(6, 6);
        force_on_origin = left_leg_thigh_jac_3_3 * (left_leg_thigh_jac_3_3.transpose() * left_leg_thigh_jac_3_3 + I6 * 0.000001).inverse() * estimated_ext_torque_lstm_.segment(0, 3);
        cout << "estimated_ext_torque_lstm_.segment(0, 3) \n"
             << estimated_ext_torque_lstm_.segment(0, 3).transpose() << endl;
        // cout<<"force_on_origin \n"<<force_on_origin.transpose()<<endl;
...
void AvatarController::poseCalibration()
{
    ...
    if ((hmd_check_pose_calibration_[0] == true) && (still_pose_cali_flag_ == false))
    {
        ...
        tracker2origin_init_[0].translation() = hmd_chest_pose_init_.translation() - hmd_pelv_pose_init_.translation();
        tracker2origin_init_[1].translation() = hmd_chest_pose_init_.translation() - hmd_lupperarm_pose_init_.translation();
        tracker2origin_init_[2].translation() = hmd_chest_pose_init_.translation() - hmd_lhand_pose_init_.translation();
        tracker2origin_init_[3].translation() = hmd_chest_pose_init_.translation() - hmd_rupperarm_pose_init_.translation();
        tracker2origin_init_[4].translation() = hmd_chest_pose_init_.translation() - hmd_rhand_pose_init_.translation();

        tracker2origin_init_[0].linear() = hmd_pelv_pose_init_.linear();
        tracker2origin_init_[1].linear() = hmd_lupperarm_pose_init_.linear();
        tracker2origin_init_[2].linear() = hmd_lhand_pose_init_.linear();
        tracker2origin_init_[3].linear() = hmd_rupperarm_pose_init_.linear();
        tracker2origin_init_[4].linear() = hmd_rhand_pose_init_.linear();
    ...
    {
        double dt_hmd_vel;
        dt_hmd_vel = rd_.control_time_ - last_hmd_vel_calc_time_;
        dt_hmd_vel = DyrosMath::minmax_cut(dt_hmd_vel, 0.0005, 0.01);

        hmd_head_vel_.segment(0, 3) = (hmd_head_pose_.translation() - hmd_head_pose_pre_.translation()) / dt_hmd_vel;
        hmd_lshoulder_vel_.segment(0, 3) = (hmd_lshoulder_pose_.translation() - hmd_lshoulder_pose_pre_.translation()) / dt_hmd_vel;
        hmd_lupperarm_vel_.segment(0, 3) = (hmd_lupperarm_pose_.translation() - hmd_lupperarm_pose_pre_.translation()) / dt_hmd_vel;
        hmd_lhand_vel_.segment(0, 3) = (hmd_lhand_pose_.translation() - hmd_lhand_pose_pre_.translation()) / dt_hmd_vel;
        ...
        // float data collect
        // hmd_chest_vel_(0) = -hmd_chest_vel_(0);
        // hmd_chest_vel_(1) = -hmd_chest_vel_(1);
        ...
        hmd_lupperarm_vel_.segment(3, 3) = ang_temp_3.axis() * ang_temp_3.angle() / dt_hmd_vel;
        hmd_lhand_vel_.segment(3, 3) = ang_temp_4.axis() * ang_temp_4.angle() / dt_hmd_vel;
        hmd_rshoulder_vel_.segment(3, 3) = ang_temp_5.axis() * ang_temp_5.angle() / dt_hmd_vel;
...
Eigen::VectorXd AvatarController::momentumObserverFbInternal(MatrixXd A_matrix, MatrixXd A_dot_matrix, VectorXd current_torque, VectorXd current_qdot, VectorXd nonlinear_effect_vector, VectorXd mob_residual_pre, VectorXd &mob_residual_integral, double dt, double k)
{
    /*
    ...
    */
    ...
    FT_H_b_inv = F.transpose() * (H_b).inverse(); // + Matrix6d::Identity()*damped_puedoinverse_eps_

    H_j = A_matrix.block(6, 6, j_dof, j_dof) - FT_H_b_inv * F;
    ...
    // H_j_dot = A_dot_matrix.block(6, 6, j_dof, j_dof);

    p_0 = nonlinear_effect_vector.segment(0, 6);
    C_j = nonlinear_effect_vector.segment(6, j_dof) - FT_H_b_inv * p_0;

    // Vector6d pelv_acc;
    // Vector3d grav_acc(0, 0, -9.81);
    // pelv_acc.segment(0, 3) = rd_.link_[Pelvis].rotm.transpose() *(rd_.q_ddot_virtual_.segment(0, 3));
    // pelv_acc.segment(3, 3) = rd_.link_[Pelvis].rotm.transpose() *(rd_.q_ddot_virtual_.segment(3, 3));

    // C_j = nonlinear_effect_vector.segment(6, j_dof) - F.transpose()*pelv_acc;

    nonlinear_term_internal = H_j_dot * current_qdot - C_j;

    // if( int(current_time_*2000)%1000 == 0 )
    // {
    //     cout<<"nonlinear_term_internal+current_torque: "<<(nonlinear_term_internal+current_torque).transpose() << endl;

    // }

    return momentumObserverCore(joint_momentum, current_torque, nonlinear_term_internal, mob_residual_pre, mob_residual_integral, dt, k);
}
Eigen::VectorXd AvatarController::momentumObserverFbExternal(MatrixXd A_matrix, MatrixXd A_dot_matrix, VectorXd current_qdot, Vector6d base_velocity, VectorXd nonlinear_effect_vector, VectorXd mob_residual_pre, VectorXd &mob_residual_integral, double dt, double k)
{
    /*
    reference: Fabrizio Flacco, Residual-based contacts estimation for humanoid robots (Humanoids 2016)
    URL: https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=7803308&tag=1
    ...
    */
    ...
    // if( int(current_time_*2000)%1000 == 0 )
    // {
    //     cout<<"nonlinear_term_external: "<<(nonlinear_term_external).transpose() << endl;

    // }

    return momentumObserverCore(base_momentum, Eigen::Vector6d::Zero(), nonlinear_term_external, mob_residual_pre, mob_residual_integral, dt, k);
}
...
MatrixXd AvatarController::getCMatrix(VectorXd q, VectorXd qdot) // TOO SLOW (~1000us for 39dof)
{
    std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();
...
MatrixXd AvatarController::getAdotMatrix(VectorXd q, VectorXd qdot) // TOO SLOW (~500us for 39dof)
{
    std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();
    ...
    Eigen::MatrixXd H_origin(dof, dof), H_new(dof, dof), H_dot(dof, dof);
    Eigen::MatrixXd m[dof];
    H_origin.setZero();
    // H_origin = A_mat_;
    H_new.setZero();
...
//         {
//             QP_leg_hqpik_.resize(hierarchy_num_leg_hqpik_);
//             QP_leg_hqpik_[i].InitializeProblemSize(variable_size_leg_hqpik_, constraint_size2_leg_hqpik_[i]);
//             J_leg_hqpik_[i].setZero(control_size_leg_hqpik_[i], variable_size_leg_hqpik_);
//             u_dot_leg_hqpik_[i].setZero(control_size_leg_hqpik_[i]);

//             ubA_leg_hqpik_[i].setZero(constraint_size2_leg_hqpik_[i]);
//             lbA_leg_hqpik_[i].setZero(constraint_size2_leg_hqpik_[i]);

//             H_leg_hqpik_[i].setZero(variable_size_leg_hqpik_, variable_size_leg_hqpik_);
//             g_leg_hqpik_[i].setZero(variable_size_leg_hqpik_);

//             ub_leg_hqpik_[i].setZero(constraint_size1_leg_hqpik_);
//             lb_leg_hqpik_[i].setZero(constraint_size1_leg_hqpik_);

//             q_dot_leg_hqpik_[i].setZero(variable_size_leg_hqpik_);

//             w1_leg_hqpik_[i] = 1;    // |A*qdot - h|
//             w2_leg_hqpik_[i] = 1e-4; // |q_dot|
//             w3_leg_hqpik_[i] = 0.0;  // acceleration (0.000)
...
//         last_solved_hierarchy_num_leg_hqpik_ = -1;
...
//     Vector3d zero3;
//     zero3.setZero();
//     J_leg_hqpik_[0].setZero();
//     J_leg_hqpik_[0].block(0, 0, 3, MODEL_DOF) = ;
//     // J_temp_.setZero(6, MODEL_DOF_VIRTUAL);
...
//     J_temp_.setZero(6, MODEL_DOF_VIRTUAL);
//     RigidBodyDynamics::CalcPointJacobian6D(model_d_, pre_desired_q_qvqd_, rd_.link_[Left_Foot].id, zero3, J_temp_, false);
//     J_leg_hqpik_[0].block(0, 0, 3, 6) = J_temp_.block(3, 6, 3, 6); // l foot position
//     J_temp_.setZero(6, MODEL_DOF_VIRTUAL);
//     RigidBodyDynamics::CalcPointJacobian6D(model_d_, pre_desired_q_qvqd_, rd_.link_[Right_Foot].id, zero3, J_temp_, false);
//     J_leg_hqpik_[0].block(3, 6, 3, 6) = J_temp_.block(3, 12, 3, 6); // r foot position
//     Vector6d error_v_leg_pos;
...
//     u_dot_leg_hqpik_[0] = 100 * error_v_leg_pos;
//     ////2nd Task
//     J_leg_hqpik_[1].setZero();
//     J_temp_.setZero(6, MODEL_DOF_VIRTUAL);
//     RigidBodyDynamics::CalcPointJacobian6D(model_d_, pre_desired_q_qvqd_, rd_.link_[Left_Foot].id, zero3, J_temp_, false);
//     J_leg_hqpik_[1].block(0, 0, 3, 6) = J_temp_.block(0, 6, 3, 6); // l foot orientation
//     J_temp_.setZero(6, MODEL_DOF_VIRTUAL);
//     RigidBodyDynamics::CalcPointJacobian6D(model_d_, pre_desired_q_qvqd_, rd_.link_[Right_Foot].id, zero3, J_temp_, false);
//     J_leg_hqpik_[1].block(3, 6, 3, 6) = J_temp_.block(0, 12, 3, 6); // r foot orientation
//     Vector6d error_w_leg_pos;
...
//     u_dot_leg_hqpik_[1] = 100 * error_w_leg_pos;

//     for (int i = 0; i < hierarchy_num_leg_hqpik_; i++)
//     {
//         if (i > last_solved_hierarchy_num_leg_hqpik_)
//         {
//             QP_leg_hqpik_[i].InitializeProblemSize(variable_size_leg_hqpik_, constraint_size2_leg_hqpik_[i]);
//         }
//     }
...
//         MatrixXd H1, H2, H3;
//         VectorXd g1, g2, g3;
//         H1 = J_leg_hqpik_[i].transpose() * J_leg_hqpik_[i];
//         H2 = Eigen::MatrixXd::Identity(variable_size_leg_hqpik_, variable_size_leg_hqpik_);
...
//         H3 = Eigen::MatrixXd::Identity(variable_size_leg_hqpik_, variable_size_leg_hqpik_) * (1 / dt_) * (1 / dt_);

//         g1 = -J_leg_hqpik_[i].transpose() * u_dot_leg_hqpik_[i];
//         g2.setZero(variable_size_leg_hqpik_);
//         g3 = -motion_q_dot_pre_.segment(0, variable_size_leg_hqpik_) * (1 / dt_) * (1 / dt_);

//         H_leg_hqpik_[i] = w1_leg_hqpik_[i] * H1 + w2_leg_hqpik_[i] * H2 + w3_leg_hqpik_[i] * H3;
//         g_leg_hqpik_[i] = w1_leg_hqpik_[i] * g1 + w2_leg_hqpik_[i] * g2 + w3_leg_hqpik_[i] * g3;

//         double speed_reduce_rate = 20; // when the current joint position is near joint limit (10 degree), joint limit condition is activated.
//         for (int j = 0; j < constraint_size1_leg_hqpik_; j++)
//         {
//             lb_leg_hqpik_[i](j) = min(max(speed_reduce_rate * (joint_limit_l_(j) - motion_q_pre_(j)), joint_vel_limit_l_(j)), joint_vel_limit_h_(j));
//             ub_leg_hqpik_[i](j) = max(min(speed_reduce_rate * (joint_limit_h_(j) - motion_q_pre_(j)), joint_vel_limit_h_(j)), joint_vel_limit_l_(j));
//         }
//         A_leg_hqpik_[i].setZero(constraint_size2_leg_hqpik_[i], variable_size_leg_hqpik_);

//         int higher_task_equality_num = 0;
//         for (int h = 0; h < i; h++)
//         {
//             A_leg_hqpik_[i].block(higher_task_equality_num, 0, control_size_leg_hqpik_[h], variable_size_leg_hqpik_) = J_leg_hqpik_[h];
//             // ubA_hqpik_[i].segment(higher_task_equality_num, control_size_hqpik_[h]) = J_hqpik_[h] * q_dot_hqpik_[h];
//             // lbA_hqpik_[i].segment(higher_task_equality_num, control_size_hqpik_[h]) = J_hqpik_[h] * q_dot_hqpik_[h];
//             ubA_leg_hqpik_[i].segment(higher_task_equality_num, control_size_leg_hqpik_[h]) = J_leg_hqpik_[h] * q_dot_leg_hqpik_[i - 1];
//             lbA_leg_hqpik_[i].segment(higher_task_equality_num, control_size_leg_hqpik_[h]) = J_leg_hqpik_[h] * q_dot_leg_hqpik_[i - 1];
//             higher_task_equality_num += control_size_leg_hqpik_[h];
//         }
//         // foot velocity constraints
//         if (i == 0)
//         {
//             A_leg_hqpik_[i].block(higher_task_equality_num, 0, 6, variable_size_leg_hqpik_) = J_leg_hqpik_[0];     // position
//             A_leg_hqpik_[i].block(higher_task_equality_num + 6, 0, 6, variable_size_leg_hqpik_) = J_leg_hqpik_[1]; // orientation

//             for (int j = 0; j < 3; j++)
//             {
//                 // linear velocity limit
//                 lbA_leg_hqpik_[i](higher_task_equality_num + j) = -3;
//                 ubA_leg_hqpik_[i](higher_task_equality_num + j) = 3;
//                 lbA_leg_hqpik_[i](higher_task_equality_num + j + 3) = -3;
//                 ubA_leg_hqpik_[i](higher_task_equality_num + j + 3) = 3;

//                 // angular velocity limit
//                 lbA_leg_hqpik_[i](higher_task_equality_num + j + 6) = -2 * M_PI;
//                 ubA_leg_hqpik_[i](higher_task_equality_num + j + 6) = 2 * M_PI;
//                 lbA_leg_hqpik_[i](higher_task_equality_num + j + 9) = -2 * M_PI;
//                 ubA_leg_hqpik_[i](higher_task_equality_num + j + 9) = 2 * M_PI;
//             }
//         }
//         else
//         {
//             A_leg_hqpik_[i].block(higher_task_equality_num, 0, 6, variable_size_leg_hqpik_) = J_leg_hqpik_[1]; // orientation

//             for (int j = 0; j < 3; j++)
//             {
//                 // angular velocity limit
//                 lbA_leg_hqpik_[i](higher_task_equality_num + j + 0) = -2 * M_PI;
//                 ubA_leg_hqpik_[i](higher_task_equality_num + j + 0) = 2 * M_PI;
//                 lbA_leg_hqpik_[i](higher_task_equality_num + j + 3) = -2 * M_PI;
//                 ubA_leg_hqpik_[i](higher_task_equality_num + j + 3) = 2 * M_PI;
//             }
//         }

//         // QP_leg_hqpik_[i].SetPrintLevel(PL_NONE);
//         QP_leg_hqpik_[i].EnableEqualityCondition(equality_condition_eps_);
//         QP_leg_hqpik_[i].UpdateMinProblem(H_leg_hqpik_[i], g_leg_hqpik_[i]);
//         // QP_leg_hqpik_[i].DeleteSubjectToAx();
//         // QP_leg_hqpik_[i].DeleteSubjectToX();
//         QP_leg_hqpik_[i].UpdateSubjectToAx(A_leg_hqpik_[i], lbA_leg_hqpik_[i], ubA_leg_hqpik_[i]);
//         QP_leg_hqpik_[i].UpdateSubjectToX(lb_leg_hqpik_[i], ub_leg_hqpik_[i]);
//         // cout<<"test8"<<endl;
//         if (QP_leg_hqpik_[i].SolveQPoases(200, qpres_leg_hqpik_))
//         {
//             q_dot_leg_hqpik_[i] = qpres_leg_hqpik_.segment(0, variable_size_leg_hqpik_);

//             last_solved_hierarchy_num_leg_hqpik_ = i;
...
//                     std::cout << "last solved q_dot: " << q_dot_leg_hqpik_[0].transpose() << std::endl;
...
//                     std::cout << "A_leg_hqpik_[0]: \n"
//                               << A_leg_hqpik_[0] << std::endl;
//                     std::cout << "com_desired_: " << com_desired_.transpose() << std::endl;
//                     std::cout << "cp_desired_: " << cp_desired_.transpose() << std::endl;

//                     temp_test = false;
//                 }
//             }
//             q_dot_leg_hqpik_[i].setZero();
//             // cout<<"Error qpres_: \n"<< qpres_ << endl;
//             break;
//         }
//         // cout<<"ubA_[0]: " << ubA_[0]<<endl;
...
//     // desired_q.setZero();
//     // desired_q_dot.setZero();
//     int q_dot_des_hierarchy = DyrosMath::minmax_cut(last_solved_hierarchy_num_leg_hqpik_, 0, hierarchy_num_leg_hqpik_ - 1);
//     if (q_dot_des_hierarchy == 0)
//     {
//         cout << "q dot zero" << endl;
//     }

//     for (int i = 0; i < variable_size_leg_hqpik_; i++)
//     {
//         desired_q_dot(i) = q_dot_leg_hqpik_[q_dot_des_hierarchy](i);
...
void AvatarController::computeLeg_QPIK(Eigen::Isometry3d lfoot_t_des, Eigen::Isometry3d rfoot_t_des, Eigen::VectorQd &desired_q, Eigen::VectorQd &desired_q_dot)
{
    if (first_loop_leg_qpik_)
    {
        ...
        w2_leg_qpik_ = 1e-6; // |q_dot|
        w3_leg_qpik_ = 0.0;  // acceleration (0.000)
    ...
    // H2 = A_mat_.block(6, 6, variable_size_leg_qpik_, variable_size_leg_qpik_) + Eigen::MatrixXd::Identity(variable_size_leg_qpik_, variable_size_leg_qpik_) * (2e-2);
    H3 = Eigen::MatrixXd::Identity(variable_size_leg_qpik_, variable_size_leg_qpik_) * (1 / dt_) * (1 / dt_);

    g1 = -J_leg_qpik_.transpose() * u_dot_leg_qpik_;
    g2.setZero(variable_size_leg_qpik_);
    g3 = -motion_q_dot_pre_.segment(0, variable_size_leg_qpik_) * (1 / dt_) * (1 / dt_);
...
void AvatarController::BoltController_MJ()
{
    ...
    double w1_step = 1.0, w2_step = 0.05, w3_step = 1.0;
    double u0_x = 0;
    double u0_y = 0;
    double b_nom_x = 0;
    double b_nom_y = 0;
    ...
    H_step(1, 1) = 200.0 * w1_step; // U_T,y (step position in y-direction)200
    H_step(2, 2) = w2_step;         // tau (step timing)
    H_step(3, 3) = w3_step;         // DCM offset in x
    H_step(4, 4) = 0.01 * w3_step;  // DCM offset in y 0.01
...
{
    // reference paper:  //https://doi.org/10.1163/016918610X493552
    ...
    {
        cout << "WARNING: Wieber MPC X is not solved !!" << endl;
        QP_mpc_x_.InitializeProblemSize(N, N);
    }
    ...
    {
        cout << "WARNING: Wieber MPC Y is not solved !!" << endl;
        QP_mpc_y_.InitializeProblemSize(N, N);
    }
...
void AvatarController::collectRobotInputData_acc_version()
{
    ...
    left_leg_mob_lstm_.robot_input_data(4) = q_virtual_Xd_global_noise_(6); // q
    left_leg_mob_lstm_.robot_input_data(5) = q_virtual_Xd_global_noise_(7);
    left_leg_mob_lstm_.robot_input_data(6) = q_virtual_Xd_global_noise_(8);
    left_leg_mob_lstm_.robot_input_data(7) = q_virtual_Xd_global_noise_(9);
    left_leg_mob_lstm_.robot_input_data(8) = q_virtual_Xd_global_noise_(10);
    left_leg_mob_lstm_.robot_input_data(9) = q_virtual_Xd_global_noise_(11);

    left_leg_mob_lstm_.robot_input_data(10) = q_dot_virtual_Xd_global_noise_(3); // pelv ang vel
    left_leg_mob_lstm_.robot_input_data(11) = q_dot_virtual_Xd_global_noise_(4);
    left_leg_mob_lstm_.robot_input_data(12) = q_dot_virtual_Xd_global_noise_(5);

    left_leg_mob_lstm_.robot_input_data(13) = q_dot_virtual_Xd_global_noise_(6); // qdot
    left_leg_mob_lstm_.robot_input_data(14) = q_dot_virtual_Xd_global_noise_(7);
    left_leg_mob_lstm_.robot_input_data(15) = q_dot_virtual_Xd_global_noise_(8);
    left_leg_mob_lstm_.robot_input_data(16) = q_dot_virtual_Xd_global_noise_(9);
    left_leg_mob_lstm_.robot_input_data(17) = q_dot_virtual_Xd_global_noise_(10);
    left_leg_mob_lstm_.robot_input_data(18) = q_dot_virtual_Xd_global_noise_(11);

    left_leg_mob_lstm_.robot_input_data(19) = rd_.torque_desired(0); // desired torque
    left_leg_mob_lstm_.robot_input_data(20) = rd_.torque_desired(1);
    left_leg_mob_lstm_.robot_input_data(21) = rd_.torque_desired(2);
    left_leg_mob_lstm_.robot_input_data(22) = rd_.torque_desired(3);
    left_leg_mob_lstm_.robot_input_data(23) = rd_.torque_desired(4);
    left_leg_mob_lstm_.robot_input_data(24) = rd_.torque_desired(5);

    left_leg_mob_lstm_.robot_input_data(25) = q_ddot_virtual_Xd_global_noise_(0); // lin acc
    left_leg_mob_lstm_.robot_input_data(26) = q_ddot_virtual_Xd_global_noise_(1);
    left_leg_mob_lstm_.robot_input_data(27) = q_ddot_virtual_Xd_global_noise_(2);
    ...
    right_leg_mob_lstm_.robot_input_data(4) = rd_.q_virtual_(12); // q
    right_leg_mob_lstm_.robot_input_data(5) = rd_.q_virtual_(13);
    right_leg_mob_lstm_.robot_input_data(6) = rd_.q_virtual_(14);
    right_leg_mob_lstm_.robot_input_data(7) = rd_.q_virtual_(15);
    right_leg_mob_lstm_.robot_input_data(8) = rd_.q_virtual_(16);
    right_leg_mob_lstm_.robot_input_data(9) = rd_.q_virtual_(17);

    right_leg_mob_lstm_.robot_input_data(10) = rd_.q_dot_virtual_(3); // pelv ang vel
    right_leg_mob_lstm_.robot_input_data(11) = rd_.q_dot_virtual_(4);
    right_leg_mob_lstm_.robot_input_data(12) = rd_.q_dot_virtual_(5);

    right_leg_mob_lstm_.robot_input_data(13) = rd_.q_dot_virtual_(12); // qdot
    right_leg_mob_lstm_.robot_input_data(14) = rd_.q_dot_virtual_(13);
    right_leg_mob_lstm_.robot_input_data(15) = rd_.q_dot_virtual_(14);
    right_leg_mob_lstm_.robot_input_data(16) = rd_.q_dot_virtual_(15);
    right_leg_mob_lstm_.robot_input_data(17) = rd_.q_dot_virtual_(16);
    right_leg_mob_lstm_.robot_input_data(18) = rd_.q_dot_virtual_(17);

    right_leg_mob_lstm_.robot_input_data(19) = rd_.torque_desired(6); // desired torque
    right_leg_mob_lstm_.robot_input_data(20) = rd_.torque_desired(7);
    right_leg_mob_lstm_.robot_input_data(21) = rd_.torque_desired(8);
    right_leg_mob_lstm_.robot_input_data(22) = rd_.torque_desired(9);
    right_leg_mob_lstm_.robot_input_data(23) = rd_.torque_desired(10);
    right_leg_mob_lstm_.robot_input_data(24) = rd_.torque_desired(11);

    right_leg_mob_lstm_.robot_input_data(25) = rd_.q_ddot_virtual_(0); // lin acc
    right_leg_mob_lstm_.robot_input_data(26) = rd_.q_ddot_virtual_(1);
    right_leg_mob_lstm_.robot_input_data(27) = rd_.q_ddot_virtual_(2);
...
void AvatarController::printOutTextFile()
{
    ...
    //         << lfoot_transform_current_from_support_.translation()(0) << "\t" << lfoot _transform_current_from_support_.translation()(1) << "\t" << lfoot_transform_current_from_support_.translation()(2) << "\t" //4
    ...
    if (walking_tick_mj % 20 == 0)
    {
        ...
        {
            file[1] << rd_.control_time_ << "\t" << lfoot_local_current_.translation()(0) << "\t" << lfoot_local_current_.translation()(1) << "\t" << lfoot_local_current_.translation()(2) << "\t";
            file[1] << rfoot_local_current_.translation()(0) << "\t" << rfoot_local_current_.translation()(1) << "\t" << rfoot_local_current_.translation()(2) << "\t";
            ...
            for (int i = 0; i < 3; i++)
            {
                file[0] << rd_.link_[Pelvis].rotm(i, 0) << "\t";    //Pelv ori first column
            }
            for (int i = 0; i < 3; i++)
            {
                file[0] << rd_.link_[Pelvis].rotm(i, 1) << "\t";    //Pelv ori second column
            }
            for (int i = 0; i < 3; i++)
            {
                file[0] << rd_.link_[Pelvis].rotm(i, 2) << "\t";    //Pelv ori third column
            }
            for (int i = 6; i < 18; i++)
            {
                file[0] << q_virtual_Xd_local_(i) << "\t";
            }
            for (int i = 0; i < 18; i++)
            {
                file[0] << q_dot_virtual_Xd_local_(i) << "\t";
            }
            for (int i = 0; i < 18; i++)
            {
            ...
            for (int i = 0; i < 18; i++)
            {
                file[0] << q_ddot_virtual_Xd_local_(i) << "\t";
            }
            for (int i = 0; i < 12; i++)
            {
                file[0] << ref_q_(i) << "\t";
            ...
            for (int i = 0; i < 6; i++)
            {
                file[0] << l_ft_wo_fw_lpf_(i) << "\t";
            }
            for (int i = 0; i < 6; i++)
            {
                file[0] << r_ft_wo_fw_lpf_(i) << "\t";
            }
            for (int i = 0; i < 6; i++)
            {
                file[0] << torque_from_l_ft_(i) << "\t";
            }
            for (int i = 0; i < 6; i++)
            {
                file[0] << torque_from_r_ft_(6 + i) << "\t";
            }
            ...
            for (int i = 0; i < 12; i++)
            {
                file[3] << estimated_model_unct_torque_slow_(i) << "\t";
            }
            for (int i = 0; i < 12; i++)
            {
                file[3] << estimated_model_unct_torque_variance_slow_(i) << "\t";
            }
            for (int i = 0; i < 6; i++)
            {
                file[3] << estimated_ext_force_lfoot_lstm_(i) << "\t";
            }
            for (int i = 0; i < 6; i++)
            {
...
void AvatarController::getRobotState()
{
    ...
    {
        atb_hmd_vel_update_ = true;
        hmd_chest_vel_slow_ = hmd_chest_vel_thread_;
        atb_hmd_vel_update_ = false;
    }
    ...
    hmd_chest_vel_slow_lpf_ = DyrosMath::lpf<6>(hmd_chest_vel_slow_, hmd_chest_vel_slow_lpf_, hz_, 8);
    ...
    // q_ddot_virtual_Xd_global_.segment(0, 3) = rd_.link_[Pelvis].rotm * rd_.imu_lin_acc;
    q_ddot_virtual_Xd_global_.segment(0, 3) = rd_.q_ddot_virtual_.segment(0, 3);
    q_ddot_virtual_Xd_global_.segment(3, 3) = rd_.q_ddot_virtual_.segment(3, 3);
    ...
    q_dot_virtual_Xd_local_.segment(0, 3) = rd_.link_[Pelvis].rotm.transpose()*rd_.q_dot_virtual_.segment(0, 3);
    q_dot_virtual_Xd_local_.segment(3, 3) = rd_.link_[Pelvis].rotm.transpose()*rd_.q_dot_virtual_.segment(3, 3);

    q_ddot_virtual_Xd_local_ = (q_dot_virtual_Xd_local_ - q_dot_virtual_Xd_local_pre_) * hz_;
    
    q_ddot_virtual_Xd_local_.segment(0, 3) = rd_.imu_lin_acc;
    q_ddot_virtual_Xd_local_.segment(3, 3) = rd_.link_[Pelvis].rotm.transpose()*rd_.q_ddot_virtual_.segment(3, 3);
    ...
    lfoot_local_current_.translation() = RigidBodyDynamics::CalcBodyToBaseCoordinates(model_local_, q_virtual_Xd_local_, rd_.link_[Left_Foot].id, Eigen::Vector3d::Zero(), false);
    lfoot_local_current_.linear() = (RigidBodyDynamics::CalcBodyWorldOrientation(model_local_, q_virtual_Xd_local_, rd_.link_[Left_Foot].id, false)).transpose();
    rfoot_local_current_.translation() = RigidBodyDynamics::CalcBodyToBaseCoordinates(model_local_, q_virtual_Xd_local_, rd_.link_[Right_Foot].id, Eigen::Vector3d::Zero(), false);
    rfoot_local_current_.linear() = (RigidBodyDynamics::CalcBodyWorldOrientation(model_local_, q_virtual_Xd_local_, rd_.link_[Right_Foot].id, false)).transpose();
    ...
    for (int i = 0; i < MODEL_DOF; i++)
    {
        // torque_current_elmo_(i) = rd_.torque_elmo_(i) / torque_nm2cnt_(i); // exterted current torque in real robot
        torque_current_elmo_(i) = rd_.torque_elmo_(i); // joint acceleration in simulation
    }
    ...
    Wrench_foot_plate(2) = -foot_plate_mass * 9.80;
    ...
    // l_ft_wo_fw_ = l_ft_ - adt2.transpose() * inertia_foot_plate * (rotrf.transpose() * lfoot_acceleration);
    ...
    // r_ft_wo_fw_ = r_ft_ - adt2.transpose() * inertia_foot_plate * (rotrf.transpose() * rfoot_acceleration);
...
void AvatarController::calculateFootStepTotal_reactive(double target_x, bool is_right_foot_swing)
{
    double initial_rot = 0.0;
    double final_rot = 0.0;
    double initial_drot = 0.0;
    ...
    unsigned int final_total_step_number = final_rot / final_drot;
    double final_residual_angle = final_rot - final_total_step_number * final_drot;
    double length_to_target = abs(target_x);
    double dlength = step_length_x_;
    unsigned int middle_total_step_number = length_to_target / abs(dlength);
    double middle_residual_length = (length_to_target - middle_total_step_number * abs(dlength)) * DyrosMath::sign(dlength);
    ...
    foot_step_(0, 0) = 0.0;
    foot_step_(0, 1) = is_right * (0.1025 + step_width);
    foot_step_(0, 6) = 0.5 + 0.5 * (-is_right); // left foot support = 1, right foot support = 0

    // first landing step
    foot_step_(1, 0) = target_x - 0.03;
    foot_step_(1, 1) = -is_right * (0.1025 + step_width);
    foot_step_(1, 6) = 0.5 + 0.5 * (is_right);

    // second step
    foot_step_(2, 0) = third_foot_step_x;
...
void AvatarController::getZmpTrajectory()
{
    ...
    if (current_step_num_ >= total_step_num_ - planning_step_number)
        norm_size = (t_last_ - t_start_ + 1) * (total_step_num_ - current_step_num_ +1) + 3.0 * hz_;
    else
                                         ...
                                         Eigen::MatrixXd Gi, Eigen::VectorXd Gd, Eigen::MatrixXd Gx, Eigen::MatrixXd A, Eigen::VectorXd B, Eigen::MatrixXd C, Eigen::Vector3d &XD, Eigen::Vector3d &YD)
{
    ...
    // cout<<"normsize: "<<ref_zmp_mj_.rows()<<"tick: "<<tick<<"total_t_: "<< t_total_<<endl;
    for (int i = 0; i < NL; i++)
    {
...
void AvatarController::getComTrajectory_mpc()
{
    ...
    {
        // cout<<"com_desired_: "<<com_desired_.transpose()<<endl;
        // cout<<"ref_zmp_mj_: "<<ref_zmp_mj_<<endl;
    }

    // MJ_graph << x_mpc_i_(0) << "," << y_mpc_i_(0) << "," << ZMP_X_REF << "," << ZMP_Y_REF << "," << cp_desired_(0) << "," << cp_desired_(1) << endl;
...
{ // 기존 알고리즘에서 바꾼거 : 0. previewcontroller에서 ZMP_Y_REF 변수 추가 1. zmp offset 2. getrobotstate에서 LPF 3. supportToFloatPattern 함수 4. Tau_CP -> 0  5. getfoottrajectory에서 발의 Euler angle
    ...
    F_T_L_x_input_dot = -0.015 * (Tau_L_x - l_ft_LPF(3)) - 50.0 * F_T_L_x_input;
    F_T_L_x_input = F_T_L_x_input + F_T_L_x_input_dot * del_t;
    //   F_T_L_x_input = 0;
    //   F_T_R_x_input_dot = -0.015*(Tau_R_x - r_ft_LPF(3)) - Kr_roll*F_T_R_x_input;
    F_T_R_x_input_dot = -0.015 * (Tau_R_x - r_ft_LPF(3)) - 50.0 * F_T_R_x_input;
    ...
    F_T_L_y_input_dot = 0.005 * (Tau_L_y - l_ft_LPF(4)) - 50.0 * F_T_L_y_input;
    F_T_L_y_input = F_T_L_y_input + F_T_L_y_input_dot * del_t;
    //   F_T_L_y_input = 0;
    //   F_T_R_y_input_dot = 0.005*(Tau_R_y - r_ft_LPF(4)) - Kr_pitch*F_T_R_y_input;
    F_T_R_y_input_dot = 0.005 * (Tau_R_y - r_ft_LPF(4)) - 50.0 * F_T_R_y_input;

...
void AvatarController::computeFast()
{
    ...
    else if (rd_.tc_.mode == 11)
    {
        ...
        // std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();
        calculateLstmOutput(left_leg_mob_lstm_);    //200~250us
        // std::chrono::steady_clock::time_point t2 = std::chrono::steady_clock::now();
...
void AvatarController::initializeLegLSTM(LSTM &lstm)
{
    ...
    lstm.buffer_tail = 0;
    lstm.input_slow.setZero(nn_input_size_);
    lstm.input_fast.setZero(nn_input_size_);
    lstm.input_thread.setZero(nn_input_size_);
    ...
    lstm.hidden.setZero(n_hidden_);
    lstm.cell.setZero(n_hidden_);
...
void AvatarController::loadLstmWeights(LSTM &lstm, std::string folder_path)
{
    ...
    while (!network_weights_file_[0].eof())
    {
        ...
        if (temp != '\n')
        {
            ...
            {
                cout<<"WARNING: W_ih has NaN value!"<<endl;
            }
        }
    }
...
void AvatarController::calculateLstmInput(LSTM &lstm)
{
    ...
    for (int i = 0; i < lstm.n_sequence_length; i++)
    {
        for (int j = 0; j < lstm.n_input; j++)
        {
            ...
            lstm.input_slow(lstm.nn_input_size - j - i * lstm.n_input - 1) = (lstm.ring_buffer(buffer_idx) - lstm.input_mean(lstm.n_input - 1 - j)) / lstm.input_std(lstm.n_input - 1 - j);
        }
    }
...
void AvatarController::calculateLstmOutput(LSTM &lstm)
{
    ...
    for (int seq = 0; seq < lstm.n_sequence_length; seq++)
    {
        for (int row_idx = 0; row_idx < 4 * lstm.n_hidden; row_idx++)
        {
            ...
            for (int input_idx = 0; input_idx < lstm.n_input; input_idx++)
                lstm.gates(row_idx) += lstm.W_ih(lstm.n_input * row_idx + input_idx) * lstm.input_fast(seq * lstm.n_input + input_idx);
            for (int hidden_idx = 0; hidden_idx < lstm.n_hidden; hidden_idx++)
                lstm.gates(row_idx) += lstm.W_hh(lstm.n_hidden * row_idx + hidden_idx) * lstm.hidden(hidden_idx);
        }
        ...
        for (int row_idx = 0; row_idx < lstm.n_hidden; row_idx++)
        {
            ...
            lstm.cell(row_idx) = lstm.forget_gate(row_idx) * lstm.cell(row_idx) + lstm.input_gate(row_idx) * lstm.cell_gate(row_idx);
            lstm.hidden(row_idx) = lstm.output_gate(row_idx) * std::tanh(lstm.cell(row_idx));
        }
    }
    ...
    for (int joint = 0; joint < lstm.n_output; joint++)
    {
        ...
        for (int hidden_idx = 0; hidden_idx < lstm.n_hidden; hidden_idx++)
        {
            lstm.output(joint) += lstm.W_linear(joint * lstm.n_hidden + hidden_idx) * lstm.hidden(hidden_idx);
        }
    }
...
void AvatarController::getRobotState()
{
    ...
    // if( walking_tick_mj%20 == 0)
    // {
    //     cout<<"robot input data:"<<left_leg_mob_lstm_.robot_input_data.transpose()<<endl;
    //     cout<<"lstm input data:"<<left_leg_mob_lstm_.input_slow.transpose()<<endl;
    // }
    // std::chrono::steady_clock::time_point t2 = std::chrono::steady_clock::now();
    floatingBaseMOB(); //created by DG
    // std::chrono::steady_clock::time_point t2 = std::chrono::steady_clock::now();
    // cout<<"LSTM input calc time: "<< std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count() <<endl;
}

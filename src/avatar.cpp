...
AvatarController::AvatarController(RobotData &rd) : rd_(rd)
{
    ...
    tcb_chest_tracker_posture_sub = nh_avatar_.subscribe("/TRACKERTOCABI0", 100, &AvatarController::TcbChestTrackerCallback, this);
    tcb_pelv_tracker_posture_sub = nh_avatar_.subscribe("/TRACKERTOCABI1", 100, &AvatarController::TcbPelvTrackerCallback, this);
...
void AvatarController::computeSlow()
{
    ...
    if (rd_.tc_.mode == 10)
    {
        if (initial_flag == 0)
        {
            ...
            CAM_upper_init_q_(17) = +70.0 * DEG2RAD;
            CAM_upper_init_q_(27) = -70.0 * DEG2RAD;
            CAM_upper_init_q_(19) = -90.0 * DEG2RAD;
...
void AvatarController::computeFast()
{
    if (rd_.tc_.mode == 10)
    {
        if (initial_flag == 1)
        {
            ...
            // loadLstmWeights(left_leg_mob_lstm_, "/home/dg/catkin_ws/src/tocabi_avatar/lstm_sim/weights/left_leg/");
            // loadLstmMeanStd(left_leg_mob_lstm_, "/home/dg/catkin_ws/src/tocabi_avatar/lstm_sim/mean_std/left_leg/");

            initializeLegLSTM(right_leg_mob_lstm_);
            // loadLstmWeights(right_leg_mob_lstm_, "/home/dg/catkin_ws/src/tocabi_avatar/lstm_sim/weights/right_leg/");
            // loadLstmMeanStd(right_leg_mob_lstm_, "/home/dg/catkin_ws/src/tocabi_avatar/lstm_sim/mean_std/right_leg/");
            initial_flag = 2;
        }
    }
    else if (rd_.tc_.mode == 11)
    {
        ...
        estimated_model_unct_torque_fast_.segment(0, 6) = left_leg_mob_lstm_.real_output;
        estimated_model_unct_torque_fast_.segment(6, 6) = right_leg_mob_lstm_.real_output;
...
void AvatarController::initWalkingParameter()
{
    ...
    tcb_chest_pose_.setIdentity();
    tcb_pelv_pose_.setIdentity();
    ...
    tcb_chest_pose_raw_.setIdentity();
    tcb_pelv_pose_raw_.setIdentity();
    ...
    tcb_chest_pose_raw_last_.setIdentity();
    tcb_pelv_pose_raw_last_.setIdentity();
    ...
    tcb_chest_pose_pre_.setIdentity();
    tcb_pelv_pose_pre_.setIdentity();
    ...
    tcb_chest_vel_.setZero();
    tcb_pelv_vel_.setZero();

    tcb_pelv_vel_thread_.setZero();
    tcb_pelv_vel_slow_.setZero();
    tcb_pelv_vel_slow_lpf_.setZero();
...
void AvatarController::poseCalibration()
{
    ...
    if (hmd_tracker_status_ == true)
    {
        ...
        tcb_chest_pose_ = tcb_chest_pose_raw_;
        tcb_pelv_pose_ = tcb_pelv_pose_raw_;
        ...
        //     tcb_chest_pose_.translation() = w * tcb_chest_pose_raw_.translation() + (1 - w) * tcb_chest_pose_raw_last_.translation();
        //     tcb_pelv_pose_.translation() = w * tcb_pelv_pose_raw_.translation() + (1 - w) * tcb_pelv_pose_raw_last_.translation();
        ...
        //     Eigen::AngleAxisd tcb_chest_ang_diff(tcb_chest_pose_raw_.linear() * tcb_chest_pose_raw_last_.linear().transpose());
        //     Eigen::AngleAxisd tcb_pelv_ang_diff(tcb_pelv_pose_raw_.linear() * tcb_pelv_pose_raw_last_.linear().transpose());

        //     Eigen::Matrix3d lhand_diff_m, rhand_diff_m, lelbow_diff_m, relbow_diff_m, head_diff_m, upperbody_diff_m, pelv_diff_m, lfoot_diff_m, rfoot_diff_m, tcb_chest_diff_m, tcb_pelv_diff_m;
        ...
        //     tcb_chest_diff_m = Eigen::AngleAxisd(tcb_chest_ang_diff.angle() * w, tcb_chest_ang_diff.axis());
        //     tcb_pelv_diff_m = Eigen::AngleAxisd(tcb_pelv_ang_diff.angle() * w, tcb_pelv_ang_diff.axis());
        ...
        //     tcb_chest_pose_.linear() = tcb_chest_diff_m * tcb_chest_pose_raw_last_.linear();
        //     tcb_pelv_pose_.linear() = tcb_pelv_diff_m * tcb_pelv_pose_raw_last_.linear();
    ...
    else //false
    {
        if (hmd_tracker_status_pre_ == true)
        {
            ...
            tcb_chest_pose_raw_last_ = tcb_chest_pose_raw_;
            tcb_pelv_pose_raw_last_ = tcb_pelv_pose_raw_;
        ...
        tcb_chest_pose_ = tcb_chest_pose_raw_last_;
        tcb_pelv_pose_ = tcb_pelv_pose_raw_last_;
    }
    ...
    Eigen::Vector3d hmd_pelv_rpy, tcb_chest_rpy;
    Eigen::Matrix3d hmd_pelv_yaw_rot, tcb_chest_yaw_rot;
    Eigen::Isometry3d hmd_pelv_pose_yaw_only, tcb_chest_pose_yaw_only;

    if (hmd_check_pose_calibration_[3] == true) //calibration done
    {
        tcb_chest_pose_yaw_only.translation() = tcb_chest_pose_.translation();

        tcb_chest_rpy = DyrosMath::rot2Euler(tcb_chest_pose_.linear());
        tcb_chest_yaw_rot = DyrosMath::rotateWithZ(tcb_chest_rpy(2));
        tcb_chest_pose_yaw_only.linear() = tcb_chest_yaw_rot;
    }
    else
    {
        tcb_chest_pose_yaw_only.translation() = tcb_chest_pose_init_.translation();

        tcb_chest_rpy = DyrosMath::rot2Euler(tcb_chest_pose_init_.linear());
        tcb_chest_yaw_rot = DyrosMath::rotateWithZ(tcb_chest_rpy(2));
        tcb_chest_pose_yaw_only.linear() = tcb_chest_yaw_rot;
    }
    ...
    tcb_pelv_pose_ = tcb_chest_pose_yaw_only.inverse() * tcb_pelv_pose_;
    ...
    if ((hmd_check_pose_calibration_[0] == true) && (still_pose_cali_flag_ == false))
    {
        ...
        tcb_chest_pose_init_ = tcb_chest_pose_;
        tcb_pelv_pose_init_ = tcb_pelv_pose_;
    ...
    //HMD Velocity
    if(tcb_pelv_pose_.translation() != tcb_pelv_pose_pre_.translation())
    {
        double dt_hmd_vel;
        ...
        tcb_pelv_vel_.segment(0, 3) = (tcb_pelv_pose_.translation() - tcb_pelv_pose_pre_.translation()) / dt_hmd_vel; 
        // float data collect
        ...
        Eigen::AngleAxisd ang_temp_11(tcb_pelv_pose_.linear() * tcb_pelv_pose_pre_.linear().transpose());
        ...
        tcb_pelv_vel_.segment(3, 3) = ang_temp_11.axis() * ang_temp_11.angle() / dt_hmd_vel;
    ...
    if(atb_hmd_vel_update_ == false)
    {
        ...
        tcb_pelv_vel_thread_ = tcb_pelv_vel_;
...
void AvatarController::hmdRawDataProcessing()
{
    ...
    Vector3d lfoot_pos_deviation, rfoot_pos_deviation;
    lfoot_pos_deviation = hmd_lfoot_pose_.translation() - hmd_lfoot_pose_init_.translation();
    rfoot_pos_deviation = hmd_rfoot_pose_.translation() - hmd_rfoot_pose_init_.translation();

    //right leg motion to the left leg
    // lfoot_pos_deviation(1) = -lfoot_pos_deviation(1);

    master_lfoot_pose_raw_.translation() = lfoot_transform_start_from_global_.translation() + (lfoot_pos_deviation);
    master_rfoot_pose_raw_.translation() = rfoot_transform_start_from_global_.translation() + (rfoot_pos_deviation);
...
/*void AvatarController::computeCAMcontrol_HQP()
{
    ...
    for (int i = 0; i < hierarchy_num_camhqp_; i++)
    {
        ...
        // DG's joint limit
        // lb_camhqp_[i](0) = min(max(speed_reduce_rate * (-20*DEG2RAD - motion_q_pre_(control_joint_idx_camhqp_[0])), joint_vel_limit_l_(control_joint_idx_camhqp_[0])), joint_vel_limit_h_(control_joint_idx_camhqp_[0]));
        // ub_camhqp_[i](0) = max(min(speed_reduce_rate * (20*DEG2RAD - motion_q_pre_(control_joint_idx_camhqp_[0])), joint_vel_limit_h_(control_joint_idx_camhqp_[0])), joint_vel_limit_l_(control_joint_idx_camhqp_[0]));
        // lb_camhqp_[i](1) = min(max(speed_reduce_rate * (-20*DEG2RAD - motion_q_pre_(control_joint_idx_camhqp_[1])), joint_vel_limit_l_(control_joint_idx_camhqp_[1])), joint_vel_limit_h_(control_joint_idx_camhqp_[1]));
        // ub_camhqp_[i](1) = max(min(speed_reduce_rate * (20*DEG2RAD - motion_q_pre_(control_joint_idx_camhqp_[1])), joint_vel_limit_h_(control_joint_idx_camhqp_[1])), joint_vel_limit_l_(control_joint_idx_camhqp_[1]));
        ...
        // lb_camhqp_[i](3) = min(max(speed_reduce_rate * (-40*DEG2RAD - motion_q_pre_(control_joint_idx_camhqp_[3])), joint_vel_limit_l_(control_joint_idx_camhqp_[3])), joint_vel_limit_h_(control_joint_idx_camhqp_[3]));
        // ub_camhqp_[i](3) = max(min(speed_reduce_rate * (40*DEG2RAD - motion_q_pre_(control_joint_idx_camhqp_[3])), joint_vel_limit_h_(control_joint_idx_camhqp_[3])), joint_vel_limit_l_(control_joint_idx_camhqp_[3]));
        ...
        ub_camhqp_[i](1) = max(min(speed_reduce_rate * (10*DEG2RAD - motion_q_pre_(control_joint_idx_camhqp_[1])), joint_vel_limit_h_(control_joint_idx_camhqp_[1])), joint_vel_limit_l_(control_joint_idx_camhqp_[1]));
        lb_camhqp_[i](2) = min(max(speed_reduce_rate * (-40*DEG2RAD - motion_q_pre_(control_joint_idx_camhqp_[2])), joint_vel_limit_l_(control_joint_idx_camhqp_[2])), joint_vel_limit_h_(control_joint_idx_camhqp_[2]));
        ub_camhqp_[i](2) = max(min(speed_reduce_rate * (40*DEG2RAD - motion_q_pre_(control_joint_idx_camhqp_[2])), joint_vel_limit_h_(control_joint_idx_camhqp_[2])), joint_vel_limit_l_(control_joint_idx_camhqp_[2]));
...
void AvatarController::computeLeg_HQPIK(Eigen::Isometry3d lfoot_t_des, Eigen::Isometry3d rfoot_t_des,  Eigen::VectorQd &desired_q, Eigen::VectorQd &desired_q_dot)
{
    ...
    u_dot_leg_hqpik_[0] = 500*error_v_leg_pos;
    ////2nd Task
    J_temp_.setZero(6, MODEL_DOF_VIRTUAL);
    RigidBodyDynamics::CalcPointJacobian6D(model_d_, pre_desired_q_qvqd_, rd_.link_[Left_Foot].id, zero3, J_temp_, false);
    J_leg_hqpik_[1].block(0, 0, 3, 6) = J_temp_.block(0, 6, 3, 6); //l foot position
    J_temp_.setZero(6, MODEL_DOF_VIRTUAL);
    RigidBodyDynamics::CalcPointJacobian6D(model_d_, pre_desired_q_qvqd_, rd_.link_[Right_Foot].id, zero3, J_temp_, false);
    J_leg_hqpik_[1].block(3, 6, 3, 6) = J_temp_.block(0, 12, 3, 6); //r foot position
    ...
    u_dot_leg_hqpik_[1] = 500*error_w_leg_pos;
    ...
    for (int i = 0; i < hierarchy_num_leg_hqpik_; i++)
    {
        ...
        {
            lb_leg_hqpik_[i](j) = min(max(speed_reduce_rate * (joint_limit_l_(j) - current_q_(j)), joint_vel_limit_l_(j)), joint_vel_limit_h_(j));
            ub_leg_hqpik_[i](j) = max(min(speed_reduce_rate * (joint_limit_h_(j) - current_q_(j)), joint_vel_limit_h_(j)), joint_vel_limit_l_(j));
        }
        ...
        if (i < 2)
        {
            ...
            {
                //linear velocity limit
                lbA_leg_hqpik_[i](higher_task_equality_num + j) = -2;
                ubA_leg_hqpik_[i](higher_task_equality_num + j) = 2;
                lbA_leg_hqpik_[i](higher_task_equality_num + j + 3) = -2;
                ubA_leg_hqpik_[i](higher_task_equality_num + j + 3) = 2;
    ...
    {
        desired_q_dot(i) = q_dot_leg_hqpik_[last_solved_hierarchy_num_leg_hqpik_](i);
...
void AvatarController::collectRobotInputData_acc_version()
{
    ...
    left_leg_mob_lstm_.robot_input_data(19) = rd_.q_ddot_virtual_(0);    //lin acc
    left_leg_mob_lstm_.robot_input_data(20) = rd_.q_ddot_virtual_(1);
    left_leg_mob_lstm_.robot_input_data(21) = rd_.q_ddot_virtual_(2);
    ...
    right_leg_mob_lstm_.robot_input_data(19) = rd_.q_ddot_virtual_(0);    //lin acc
    right_leg_mob_lstm_.robot_input_data(20) = rd_.q_ddot_virtual_(1);
    right_leg_mob_lstm_.robot_input_data(21) = rd_.q_ddot_virtual_(2);
...
void AvatarController::savePreData()
{
    ...
    tcb_pelv_pose_pre_ = tcb_pelv_pose_;
    tcb_chest_pose_pre_ = tcb_chest_pose_;
...
//////TOCABI
void AvatarController::TcbChestTrackerCallback(const tocabi_msgs::matrix_3_4 &msg)
{
    tcb_chest_pose_raw_.linear()(0, 0) = msg.firstRow[0];
    tcb_chest_pose_raw_.linear()(0, 1) = msg.firstRow[1];
    tcb_chest_pose_raw_.linear()(0, 2) = msg.firstRow[2];

    tcb_chest_pose_raw_.linear()(1, 0) = msg.secondRow[0];
    tcb_chest_pose_raw_.linear()(1, 1) = msg.secondRow[1];
    tcb_chest_pose_raw_.linear()(1, 2) = msg.secondRow[2];

    tcb_chest_pose_raw_.linear()(2, 0) = msg.thirdRow[0];
    tcb_chest_pose_raw_.linear()(2, 1) = msg.thirdRow[1];
    tcb_chest_pose_raw_.linear()(2, 2) = msg.thirdRow[2];

    tcb_chest_pose_raw_.translation()(0) = msg.firstRow[3];
    tcb_chest_pose_raw_.translation()(1) = msg.secondRow[3];
    tcb_chest_pose_raw_.translation()(2) = msg.thirdRow[3];
}
void AvatarController::TcbPelvTrackerCallback(const tocabi_msgs::matrix_3_4 &msg)
{
    tcb_pelv_pose_raw_.linear()(0, 0) = msg.firstRow[0];
    tcb_pelv_pose_raw_.linear()(0, 1) = msg.firstRow[1];
    tcb_pelv_pose_raw_.linear()(0, 2) = msg.firstRow[2];

    tcb_pelv_pose_raw_.linear()(1, 0) = msg.secondRow[0];
    tcb_pelv_pose_raw_.linear()(1, 1) = msg.secondRow[1];
    tcb_pelv_pose_raw_.linear()(1, 2) = msg.secondRow[2];

    tcb_pelv_pose_raw_.linear()(2, 0) = msg.thirdRow[0];
    tcb_pelv_pose_raw_.linear()(2, 1) = msg.thirdRow[1];
    tcb_pelv_pose_raw_.linear()(2, 2) = msg.thirdRow[2];

    tcb_pelv_pose_raw_.translation()(0) = msg.firstRow[3];
    tcb_pelv_pose_raw_.translation()(1) = msg.secondRow[3];
    tcb_pelv_pose_raw_.translation()(2) = msg.thirdRow[3];
}
...
void AvatarController::printOutTextFile()
{
    ...
    if (walking_tick_mj % 20 == 0)
    {
        if (printout_cnt_ <= 100 * 60 * 60 * 9) //9h
        {
            ...
            for(int i = 0; i <3 ; i++) 
            {
                file[3]<< tcb_pelv_vel_slow_lpf_(i) << "\t";
            }
            file[3]<<endl;
...
void AvatarController::getRobotState()
{
    ...
    {
        atb_hmd_vel_update_ = true;
        tcb_pelv_vel_slow_ = tcb_pelv_vel_thread_;
        atb_hmd_vel_update_ = false;
    }


    // tcb_pelv_vel_slow_(0) = DyrosMath::minmax_cut(tcb_pelv_vel_slow_(0), -1.5, 1.5);
    // tcb_pelv_vel_slow_(1) = DyrosMath::minmax_cut(tcb_pelv_vel_slow_(1), -1.5, 1.5);
    // tcb_pelv_vel_slow_(2) = DyrosMath::minmax_cut(tcb_pelv_vel_slow_(2), -0.4, 0.4);
    

    tcb_pelv_vel_slow_lpf_ = DyrosMath::lpf<6>(tcb_pelv_vel_slow_, tcb_pelv_vel_slow_lpf_, hz_, 6);

    // if(walking_tick_mj%1000==0)
    // {
    //     cout<<"tcb_pelv_vel_slow_lpf_: "<<tcb_pelv_vel_slow_lpf_.transpose()<<endl;
    // }
    // Base frame is global
    if (walking_tick_mj == 0)
        q_dot_virtual_Xd_global_pre_ = rd_.q_dot_virtual_;
    q_dot_virtual_Xd_global_ = rd_.q_dot_virtual_;
    q_dot_virtual_Xd_global_.segment(0, 3) = tcb_pelv_vel_slow_lpf_.segment(0, 3);  //tracker
...
void AvatarController::CentroidalMomentCalculator()
{
    ...
    double foot_width_x = 0.150;  // alpha = 0.8 / original foot width in urdf -> 0.15 
    double foot_width_y = 0.065; // alpha = 0.8 / original foot width in urdf -> 0.065 
    ...
    double recovery_damping = 20; //damping 20 is equivalent to 0,99 exp gain
    ...
    else if (del_cmp(0) <= foot_width_x && del_cmp(0) > -foot_width_x)
    {
        ...
        del_tau_(1) = DyrosMath::minmax_cut(del_tau_(1), (-foot_width_x * rbs_ratio - zmp_measured_LPF_(0)) * M_G, (foot_width_x * rbs_ratio - zmp_measured_LPF_(0)) * M_G);
    }
    ...
    else if (del_cmp(1) <= foot_width_y && del_cmp(1) > -foot_width_y)
    {
        ...
        // del_tau_(0) = DyrosMath::minmax_cut( del_tau_(0), -(foot_width_y*rbs_ratio - zmp_measured_LPF_(1))*M_G, -(-foot_width_y*rbs_ratio - zmp_measured_LPF_(1))*M_G );
    }

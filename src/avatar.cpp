...
void AvatarController::computeSlow()
{
    ...
    else if (rd_.tc_.mode == 12)
    {
        ...
        // Gravity_MJ_ = wbc_.GravityCompensationTorque(rd_);
    ...
    else if (rd_.tc_.mode == 11)
    {
        ...
        else
        {
            ...
            if (atb_grav_update_ == false)
            {
                atb_grav_update_ = true;
                Gravity_MJ_ = WBC::GravityCompensationTorque(rd_);
                Gravity_MJ_ = WBC::ContactForceRedistributionTorqueWalking(rd_, Gravity_MJ_, 0.9, 1, support_foot);
                atb_grav_update_ = false;
            }
            // torque_lower_.setZero();
            // for(int i = 0; i < 12; i++)
...
void AvatarController::motionRetargeting_HQPIK()
{
    ...
    for (int i = 0; i < hierarchy_num_hqpik_; i++)
    {
        ...
        H2 = A_mat_.block(18, 18, variable_size_hqpik_, variable_size_hqpik_) + Eigen::MatrixXd::Identity(variable_size_hqpik_, variable_size_hqpik_)*(1e-5);
...
void AvatarController::Joint_gain_set_MJ()
{
    ...
    // Kp(0) = 2000.0;
    // Kd(0) = 15.0; // Left Hip yaw
    // Kp(1) = 5000.0;
    // Kd(1) = 50.0; // Left Hip roll
    // Kp(2) = 4000.0;
    // Kd(2) = 20.0; // Left Hip pitch
    // Kp(3) = 3700.0;
    // Kd(3) = 25.0; // Left Knee pitch
    // Kp(4) = 5000.0;
    // Kd(4) = 30.0; // Left Ankle pitch /5000 / 30
    // Kp(5) = 5000.0;
    // Kd(5) = 30.0; // Left Ankle roll /5000 / 30

    // Kp(6) = 2000.0;
    // Kd(6) = 15.0; // Right Hip yaw
    // Kp(7) = 5000.0;
    // Kd(7) = 50.0; // Right Hip roll
    // Kp(8) = 4000.0;
    // Kd(8) = 20.0; // Right Hip pitch
    // Kp(9) = 3700.0;
    // Kd(9) = 25.0; // Right Knee pitch
    // Kp(10) = 5000.0;
    // Kd(10) = 30.0; // Right Ankle pitch
    // Kp(11) = 5000.0;
    // Kd(11) = 30.0; // Right Ankle roll

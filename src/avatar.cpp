#include "avatar.h"
// #include "/mujoco_ros/include/mjros.h"

#include <fstream>

using namespace TOCABI;

ofstream save_p("/home/dyros/.ros/data/p.txt");
ofstream save_f("/home/dyros/.ros/data/f.txt"); // Left: fx, fy, fz, tx, ty, tz, Right:
ofstream save_num_steps("/home/dyros/.ros/data/num_steps.txt");
ofstream save_zmp("/home/dyros/.ros/data/zmp.txt");
ofstream save_foot_step("/home/dyros/.ros/data/target_swing.txt");
ofstream save_walking_result("/home/dyros/.ros/data/walking_result.txt");
ofstream save_pelv_acc_lpf("/home/dyros/.ros/data/pelv_acc_lpf.txt");
ofstream save_w_input("/home/dyros/.ros/data/w_input.txt");
ofstream save_p_input("/home/dyros/.ros/data/p_input.txt");
ofstream save_indicator("/home/dyros/.ros/data/indicator.txt");
ofstream save_cs("/home/dyros/.ros/data/cs.txt");
ofstream save_ref_cp("/home/dyros/.ros/data/ref_cp.txt");
ofstream save_cp("/home/dyros/.ros/data/cp.txt");
...
void AvatarController::computeSlow()
{
    ...
    if (rd_.tc_.mode == 10)
    {
        if (initial_flag == 0)
        {
            ...
            pelv_acc_current_LPF_ = rd_.imu_lin_acc;
        }
    ...
    else if (rd_.tc_.mode == 11)
    {
        ...
        if (walking_enable_ == true)
        {
            ...
            Eigen::Vector2d com_err;
            com_err(0) = com_desired_(0) - com_support_current_(0);
            com_err(1) = com_desired_(1) - com_support_current_(1);
            
            if(com_err.norm() > 0.10) save_flag_ = false;
            if (save_flag_ == false)
            {
                save_walking_result<<"fail"<<endl;
                save_flag_ = true;
            }
            // if(save_flag_ == true)
            saveRobotData();
            // else
            //     std::cout<<"ROBOT IS FALLEN"<<endl;
...
void AvatarController::getRobotData()
{
    ...
    pelv_pos_current_ = rd_.link_[Pelvis].xpos; // global frame
...
{
    // cout<<current_step_num_<<", "<<total_step_num_<<endl;
    ...
    {
        supportfoot_float_current_ = rfoot_float_current_; // fixed value
    }
...
void AvatarController::calculateFootStepTotal_MJ()
{
    ...
    // save global target per step index
    for(int i = 0; i < number_of_foot_step; i ++)
    {
        save_foot_step << foot_step_(i, 0) << ", "
        << foot_step_(i, 1) << ", "
        << foot_step_(i, 2) << ", "
        << foot_step_(i, 3) << ", "
        << foot_step_(i, 4) << ", "
...
void AvatarController::getFootTrajectory()
{
    ...
    else if (walking_tick_mj >= t_start_ + t_rest_init_ + t_double1_ && walking_tick_mj < t_start_ + t_total_ - t_double2_ - t_rest_last_)
    {
        ...
        if (foot_step_(current_step_num_, 6) == 1)
        {
            ...
            Eigen::Vector2d rfoot_temp, rfoot_start, rfoot_end;     
            if(current_step_num_ < 2)              
                rfoot_start << foot_step_(current_step_num_, 0), foot_step_(current_step_num_, 1);
            else
                rfoot_start << foot_step_(current_step_num_-2, 0), foot_step_(current_step_num_-2, 1);
            rfoot_end << foot_step_(current_step_num_, 0), foot_step_(current_step_num_, 1);  
            ...
            for (int i = 0; i < 2; i++){
                ...
                rfoot_temp(i) = DyrosMath::cubic(walking_tick_mj, t_start_real_ + t_double1_ + t_rest_temp, t_start_ + t_total_ - t_rest_last_ - t_double2_, rfoot_start(i), rfoot_end(i), 0.0, 0.0);
            }
        ...
        else if (foot_step_(current_step_num_, 6) == 0)
        {
            ...
            Eigen::Vector2d lfoot_temp, lfoot_start, lfoot_end;     
            if(current_step_num_ < 2)              
                lfoot_start << foot_step_(current_step_num_, 0), foot_step_(current_step_num_, 1);
            else
                lfoot_start << foot_step_(current_step_num_-2, 0), foot_step_(current_step_num_-2, 1);
            lfoot_end << foot_step_(current_step_num_, 0), foot_step_(current_step_num_, 1);            
            for (int i = 0; i < 2; i++)
            {
                ...
                lfoot_temp(i) = DyrosMath::cubic(walking_tick_mj, t_start_real_ + t_double1_ + t_rest_temp, t_start_ + t_total_ - t_rest_last_ - t_double2_, lfoot_start(i), lfoot_end(i), 0.0, 0.0);
            }
...
void AvatarController::parameterSetting()
{
    ...
    float x_ran, y_ran, theta_ran;
    x_ran = (rand() % 201) - 100;
    y_ran = (rand() % 201) - 100;
    theta_ran = (rand() % 201);
    x_ran = x_ran/100;
    y_ran = y_ran/100;
    theta_ran = theta_ran/100;
    ...
    com_height_ = 0.71;
    target_theta_ = 0.0;
    // target_theta_ = theta_ran*M_PI; //0.0;
    step_length_x_ = 0.1;
    step_length_y_ = 0.0;
...
void AvatarController::saveRobotData()
{       
    save_p << lfoot_support_current_.translation()(0) << ", "<< lfoot_float_current_.translation()(1) <<", "<<lfoot_support_current_.translation()(2)<<", "      
    << rfoot_support_current_.translation()(0) << ", "<< rfoot_support_current_.translation()(1) << "," << rfoot_support_current_.translation()(2) << std::endl;

    save_f << l_ft_LPF(0) << ", "<< l_ft_LPF(1) << ", "<< l_ft_LPF(2) << ", "
    << l_ft_LPF(3) << ", "<< l_ft_LPF(4) << ", "<< l_ft_LPF(5) << ", "
    << r_ft_LPF(0) << ", "<< r_ft_LPF(1) << ", "<< r_ft_LPF(2) << ", "
    << r_ft_LPF(3) << ", "<< r_ft_LPF(4) << ", "<< r_ft_LPF(5) << ", " << endl;

    // save_c << rd_.link_[COM_id].xpos(0)<< ", "<< rd_.link_[COM_id].xpos(1)<< ", "<< rd_.link_[COM_id].xpos(2)<<endl; // com in the global frame
    // save_c << com_pos_current_(0) << ", " << com_pos_current_(1) << ", " << com_pos_current_(2) << endl;
    
    // save_c_support << com_support_current_(0) << ", "<< com_support_current_(1) << ", " << com_support_current_(2) << endl;    
    // save_ref_c << com_desired_(0) << ","<< com_desired_(1) << ","<< com_desired_(2) << endl;            
    // save_com_r << pelv_rpy_current_(0) <<", "<< pelv_rpy_current_(1) <<", "<< pelv_rpy_current_(2) << endl;

    save_num_steps << current_step_num_ << endl;

    // save_pelv_p << pelv_support_current_.translation()(0) << ", " 
    // << pelv_support_current_.translation()(1) << ", "
    // << pelv_support_current_.translation()(2) << endl;

    // save_ref_pelv_p << pelv_trajectory_support_.translation()(0) << ", " 
    // << pelv_trajectory_support_.translation()(1) << ", " << pelv_trajectory_support_.translation()(2) << endl; 

    save_zmp << zmp_measured_LPF_(0) << ", "<< zmp_measured_LPF_(1) << endl;

    save_pelv_acc_lpf << pelv_acc_current_LPF_(0)<<", "
    << pelv_acc_current_LPF_(1)<<", "
    << pelv_acc_current_LPF_(2)<< endl;

    // save_w_input<<f_add_(0)<<", "<<f_add_(1)<<", "<<f_add_(2)<<", "<<f_add_(3)<<", "<<f_add_(4)<<", "<<f_add_(5)<<std::endl;

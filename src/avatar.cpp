...
void AvatarController::computeSlow()
{
    ...
    if (rd_.tc_.mode == 10)
    {
        if (initial_flag == 0)
        {
            ...
            CAM_upper_init_q_(17) = +80.0 * DEG2RAD; // Rolling dist +70.0 deg
            CAM_upper_init_q_(27) = -80.0 * DEG2RAD; // Rolling dist -70.0 deg
    ...
    else if (rd_.tc_.mode == 11)
    {
        ...
        if (walking_enable_ == true)
        {
            ...
            if (current_step_num_ < total_step_num_)
            {
                ...
                {
                    mujoco_applied_ext_force_.data[0] = -92.0; //x-axis linear force -87은 팔 아주조금, -88은 좀 많이씀  
...
//         // MJ's joint limit
//         lb_camhqp_[i](0) = min(max(speed_reduce_rate * (-25.0*DEG2RAD - motion_q_pre_(control_joint_idx_camhqp_[0])), joint_vel_limit_l_(control_joint_idx_camhqp_[0])), joint_vel_limit_h_(control_joint_idx_camhqp_[0]));
//         ub_camhqp_[i](0) = max(min(speed_reduce_rate * (25.0*DEG2RAD - motion_q_pre_(control_joint_idx_camhqp_[0])), joint_vel_limit_h_(control_joint_idx_camhqp_[0])), joint_vel_limit_l_(control_joint_idx_camhqp_[0]));
//         lb_camhqp_[i](1) = min(max(speed_reduce_rate * (-25.0*DEG2RAD - motion_q_pre_(control_joint_idx_camhqp_[1])), joint_vel_limit_l_(control_joint_idx_camhqp_[1])), joint_vel_limit_h_(control_joint_idx_camhqp_[1]));
//         ub_camhqp_[i](1) = max(min(speed_reduce_rate * (25.0*DEG2RAD - motion_q_pre_(control_joint_idx_camhqp_[1])), joint_vel_limit_h_(control_joint_idx_camhqp_[1])), joint_vel_limit_l_(control_joint_idx_camhqp_[1]));
//         // Left Shoulder pitch

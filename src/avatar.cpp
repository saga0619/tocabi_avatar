...
void AvatarController::floatingBaseMOB()
{
    ...
    if( int(current_time_*2000)%1000 == 0 )
    {
        // cout<<"mob_residual_internal_: \n"<<mob_residual_internal_.segment(0, 12).transpose() <<endl;
...
void AvatarController::printOutTextFile()
{
    ...
    if(walking_tick_mj%20 == 0)
    {
        if(printout_cnt_ <= 100*60*60*9) //9h
        {
            ...
            for(int i = 0; i <6 ; i++) 
            {
                file[1]<< l_ft_wo_fw_LPF_(i) << "\t";
            }
            for(int i = 0; i <6 ; i++) 
            {
                file[1]<< r_ft_wo_fw_LPF_(i) << "\t";
            }
            for(int i = 0; i <6 ; i++) 
            {
                file[1]<< l_ft_LPF(i) << "\t";
...
void AvatarController::getRobotState()
{
    ...
    l_ft_wo_fw_LPF_ = DyrosMath::lpf<6>(l_ft_wo_fw_, l_ft_wo_fw_LPF_, hz_, 100/(2*M_PI));
    ...
    r_ft_wo_fw_LPF_ = DyrosMath::lpf<6>(r_ft_wo_fw_, r_ft_wo_fw_LPF_, hz_, 100/(2*M_PI));
    // if( int(rd_.control_time_*2000)%2000 == 0)
    // {
    //     cout<<"l_ft_wo_fw_: "<<l_ft_wo_fw_.transpose()<<endl;
    //     cout<<"l_ft_wo_fw_force_norm: "<<l_ft_wo_fw_.segment(0,3).norm()<<endl;
    //     cout<<"r_ft_wo_fw_: "<<r_ft_wo_fw_.transpose()<<endl;
    //     cout<<"r_ft_wo_fw_force_norm: "<<r_ft_wo_fw_.segment(0,3).norm()<<endl;
    //     cout<<"mass*G: "<<rd_.link_[COM_id].mass * GRAVITY<<endl;
    // }
    ...
    // std::chrono::steady_clock::time_point t2 = std::chrono::steady_clock::now();
    // cout<<"floatingBaseMOB time: "<< std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count() <<endl;

    ...
    else if (rd_.tc_.mode == 11)
    {
        ...
        if (walking_enable_ == true)
        {
            ...
            if (current_step_num_ < total_step_num_)
            {
                ...
                {
                    mujoco_applied_ext_force_.data[0] = -117.0; //x-axis linear force // -75N linear force (7.5-8.0 s) only waist // -117~120N (7.5-7.8s) with arms -118 is best..
                    mujoco_applied_ext_force_.data[1] = 0.0;  //y-axis linear force // -50N linear force (7.5-7.8 s)
                ...
                else if((walking_tick_mj >= 8.6*hz_)&&(walking_tick_mj < 8.9*hz_)) // 8.6 8.9
                {
                    mujoco_applied_ext_force_.data[0] = -93.0; //x-axis linear force -87은 팔 아주조금, -88은 좀 많이씀  
...
void AvatarController::CentroidalMomentCalculator()
{
    ...
    double recovery_damping = 5.0; //damping 20 is equivalent to 0,99 exp gain // 2정도 하면 반대방향으로 치는게 15Nm, 20하면 150Nm

...
ofstream MJ_COM("/home/myeongju/MJ_COM.txt");
    ...
    else if (rd_.tc_.mode == 11)
    {
        ...
        if (walking_enable_ == true)
        {
            ...
            {                
                ...
                if(current_step_num_ == 4 && (walking_tick_mj >= t_start_ + 0.15*hz_ + 0.6*0.65*hz_)  && (walking_tick_mj < t_start_ + 0.15*hz_ + 0.6*0.65*hz_ + 0.2*hz_))
                { // -170,175 // -350 7.5 - 7.6 //  67% 
                    cout << current_step_num_ << "," << t_start_*0.0005 << "," << walking_tick_mj*0.0005 << endl;
...
{   
    ...
    dsp_scaler_dot_(0) = 250.0 * stepping_err(0) - 50.0 * dsp_scaler_(0); // 300, -10
...
{   
    ...
    desired_swing_foot_LPF_(0) = 1 / (1 + 2 * M_PI * 8.0 * del_t) * desired_swing_foot_LPF_(0) + (2 * M_PI * 8.0 * del_t) / (1 + 2 * M_PI * 8.0 * del_t) * desired_swing_foot(0);
    desired_swing_foot_LPF_(1) = 1 / (1 + 2 * M_PI * 8.0 * del_t) * desired_swing_foot_LPF_(1) + (2 * M_PI * 8.0 * del_t) / (1 + 2 * M_PI * 8.0 * del_t) * desired_swing_foot(1);
...
void AvatarController::getComTrajectory_mpc()
{
    ...
    MJ_COM << com_desired_(0) << "," << com_desired_(1) << "," << com_support_current_(0) << "," << com_support_current_(1) << endl;     
...
{ // 기존 알고리즘에서 바꾼거 : 0. previewcontroller에서 ZMP_Y_REF 변수 추가 1. zmp offset 2. getrobotstate에서 LPF 3. supportToFloatPattern 함수 4. Tau_CP -> 0  5. getfoottrajectory에서 발의 Euler angle
    ...
    // del_zmp(0) = DyrosMath::minmax_cut(del_zmp(0), -0.09, 0.09);
    // del_zmp(1) = DyrosMath::minmax_cut(del_zmp(1), -0.06, 0.06); 
